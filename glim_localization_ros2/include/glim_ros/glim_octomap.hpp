...
class GlimOctomap : public ExtensionModuleROS2 {
  ...
  inline bool mapChanged(const MapMetaData& old_map_info,
                         const MapMetaData& new_map_info) {
  ...
  void insert_pointcloud_to_octomap(gtsam_points::PointCloud::ConstPtr submap,
                                    const Eigen::Isometry3d& pose);
  void handlePreNodeTraversal(const rclcpp::Time& rostime);
  bool isSpeckleNode(const octomap::OcTreeKey& n_key) const;

  /// hook that is called when traversing all nodes of the updated Octree (does
  /// nothing here)
  virtual void handleNode([[maybe_unused]] const OcTreeT::iterator& it) {}

  /// hook that is called
  /// when traversing all nodes of the updated Octree in the updated area (does
  /// nothing here)
  ...
  /// hook that is called
  /// when traversing occupied nodes in the updated area (updates 2D map
  /// projection here)
  ...
  /// hook that is called
  /// when traversing free nodes in the updated area (updates 2D map projection
  /// here)
