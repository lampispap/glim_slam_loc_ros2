...
RvizViewer::RvizViewer() : logger(create_module_logger("rviz")) {
  ...
  lidar_frame_id =
          config.param<std::string>("glim_ros", "lidar_frame_id", "lidar");
  ...
  odom_frame_id =
          config.param<std::string>("glim_ros", "odom_frame_id", "odom");
  map_frame_id = config.param<std::string>("glim_ros", "map_frame_id", "map");
  publish_imu2lidar =
          config.param<bool>("glim_ros", "publish_imu2lidar", false);
  publish_active_submaps =
          config.param<bool>("glim_ros", "publish_active_submaps", false);
  tf_time_offset = config.param<double>("glim_ros", "tf_time_offset", 1e-6);
  rviz_random_sampling_rate =
          config.param<double>("glim_ros", "rviz_random_sampling_rate", 0.1);

  Config sensor_config(GlobalConfig::get_config_path("config_sensors"));
  const auto T_lidar_imu = sensor_config.param<Eigen::Isometry3d>(
...
std::vector<GenericTopicSubscription::Ptr> RvizViewer::create_subscriptions(
        rclcpp::Node& node) {
  ...
  points_pub =
          node.create_publisher<sensor_msgs::msg::PointCloud2>("~/points", 10);
  aligned_points_pub = node.create_publisher<sensor_msgs::msg::PointCloud2>(
          "~/aligned_points", 10);
  ...
  rmw_qos_profile_t map_qos_profile = {
          ...
          false};
  rclcpp::QoS map_qos(rclcpp::QoSInitialization(map_qos_profile.history,
                                                map_qos_profile.depth),
                      map_qos_profile);
  map_pub = node.create_publisher<sensor_msgs::msg::PointCloud2>("~/map",
                                                                 map_qos);
  submap_pub = node.create_publisher<sensor_msgs::msg::PointCloud2>(
          "~/submap_debug", map_qos);
  odom_pub = node.create_publisher<nav_msgs::msg::Odometry>("~/odom", 10);
  user_event_pub =
          node.create_publisher<std_msgs::msg::Header>("~/user_event", 10);
  pose_pub =
          node.create_publisher<geometry_msgs::msg::PoseStamped>("~/pose", 10);
  point_pub = node.create_publisher<geometry_msgs::msg::Point>(
          "~/relocalize_point", 1);
  submap_pose_pub = node.create_publisher<geometry_msgs::msg::PoseStamped>(
          "~/submap_pose", 10);
...
void RvizViewer::set_callbacks() {
  ...
  OdometryEstimationCallbacks::on_new_frame.add(
          std::bind(&RvizViewer::odometry_new_frame, this, _1));
  LocalizationCallbacks::on_update_localization_submaps.add(
          std::bind(&RvizViewer::on_localization_submap, this, _1));
  LocalizationCallbacks::on_update_submap_initial_pose.add(
          std::bind(&RvizViewer::on_submap_debug, this, _1, _2));
  GlobalMappingCallbacks::on_update_submaps.add(
          std::bind(&RvizViewer::globalmap_on_update_submaps, this, _1));
  LocalizationCallbacks::on_update_active_submaps.add(
          std::bind(&RvizViewer::on_update_active_submaps, this, _1));
  ViewerCallbacks::user_event.add(
          std::bind(&RvizViewer::on_user_event, this, _1));
  ViewerCallbacks::on_load_map.add(
          std::bind(&RvizViewer::on_user_load_map, this));
  ...
  SubMappingCallbacks::on_set_gps_origin.add([this](double lat, double lon) {
    ...
    logger->info("UTM Initialized: North {}, East {}, Zone {}", UTMNorthing,
                 UTMEasting, UTMZone);
...
void RvizViewer::on_user_load_map() {
  ...
  // Wait for the result.
  const std::future_status load_map_status =
...
void RvizViewer::odometry_new_frame(
  ...
  if (base_frame_id == imu_frame_id) {
    ...
    try {
      // const auto trans_imu_base = tf_buffer->lookupTransform(imu_frame_id,
      // base_frame_id, rclcpp::Time(0.), timeout); const auto& t =
      // trans_imu_base.transform.translation; const auto& r =
      // trans_imu_base.transform.rotation;
      //
      // Eigen::Isometry3d T_imu_base = Eigen::Isometry3d::Identity();
      // T_imu_base.translation() << t.x, t.y, t.z;
      // T_imu_base.linear() = Eigen::Quaterniond(r.w, r.x, r.y,
      // r.z).toRotationMatrix();
      ...
      logger->warn("Failed to lookup transform from {} to {} (stamp={}.{}): {}",
                   imu_frame_id, base_frame_id, stamp.sec, stamp.nanosec,
                   e.what());
    }
  }
  ...
  if (odom_pub->get_subscription_count()) {
    ...
    if (base_frame_id != imu_frame_id) {
      try {
        const auto trans_imu_base = tf_buffer->lookupTransform(
                imu_frame_id, base_frame_id, rclcpp::Time(0.), timeout);
        ...
        T_imu_base.translation() << t.x, t.y, t.z;
        T_imu_base.linear() =
                Eigen::Quaterniond(r.w, r.x, r.y, r.z).toRotationMatrix();

        T_odom_base = T_odom_imu * T_imu_base;
      } catch (const tf2::TransformException& e) {
        logger->warn(
                "Failed to lookup transform from {} to {} (stamp={}.{}): {}",
                imu_frame_id, base_frame_id, stamp.sec, stamp.nanosec,
                e.what());
      }
    }
  ...
  if (points_pub->get_subscription_count()) {
    ...
    auto points =
            frame_to_pointcloud2(frame_id, new_frame->stamp, *new_frame->frame);
    points_pub->publish(*points);

    logger->debug("published points (stamp={} num_points={})", new_frame->stamp,
                  new_frame->frame->size());
  }

  if (aligned_points_pub->get_subscription_count()) {
    // Publish points aligned to the world frame to avoid some visualization
    // issues in Rviz2
    std::vector<Eigen::Vector4d> transformed(new_frame->frame->size());
    for (int i = 0; i < new_frame->frame->size(); i++) {
      transformed[i] =
              new_frame->T_world_sensor() * new_frame->frame->points[i];
    }
    ...
    logger->debug("published aligned_points (stamp={} num_points={})",
                  new_frame->stamp, frame.size());
  }
}

void RvizViewer::on_submap_debug(gtsam_points::PointCloud::ConstPtr frame,
  ...
  invoke([this, frame, submap_pose] {
    ...
    const rclcpp::Time now =
            rclcpp::Clock(rcl_clock_type_t::RCL_ROS_TIME).now();
    auto points_msg =
            frame_to_pointcloud2(map_frame_id, now.seconds(), *merged);
    this->map_pub->publish(*points_msg);

    // Publish sensor pose (with loop closure)
    geometry_msgs::msg::PoseStamped pose;
    Eigen::Quaterniond quat_world_imu =
            Eigen::Quaterniond(submap_pose.linear());
...
void RvizViewer::on_localization_submap(
        const std::vector<SubMap::Ptr>& prebuilt_submaps) {
  invoke([this, prebuilt_submaps] {
    ...
    for (int i = 0; i < prebuilt_submaps.size(); i++) {
      ...
      std::transform(submap->frame->points,
                     ...
                     merged->points + begin, [&](const Eigen::Vector4d& p) {
                       return submap->T_world_origin * p;
                     });
      begin += submap->frame->size();
    }

    auto downsampled = gtsam_points::random_sampling(
            merged, rviz_random_sampling_rate, mt);

    const rclcpp::Time now =
            rclcpp::Clock(rcl_clock_type_t::RCL_ROS_TIME).now();
    auto points_msg =
            frame_to_pointcloud2(map_frame_id, now.seconds(), *downsampled);
    submap_pub->publish(*points_msg);
  });
}

void RvizViewer::on_update_active_submaps(
        const std::map<int, SubMap::Ptr>& submaps) {
  ...
  // submap_poses(submaps.size()); for (int i = 0; i < submaps.size(); i++) {
  ...
  invoke([this] {
    ...
    // logger->warn("Publishing global map is computationally demanding and not
    // recommended");
    int total_num_points = 0;
    ...
    //   std::transform(submap->points, submap->points + submap->size(),
    //   merged->points + begin, [&](const Eigen::Vector4d& p) {
    //      return submap->T_world_origin * p; });
    //   begin += submap->size();
    // }
    ...
    // std::transform(submap->points, submap->points + submap->size(),
    // merged->points + begin,
    //    [&](const Eigen::Vector4d& p) { return latest_submap->T_world_origin *
    //    p; });
...
void RvizViewer::globalmap_on_update_submaps(
  ...
  if (publish_active_submaps) {
    ...
    invoke([this] {
      ...
      // logger->warn("Publishing global map is computationally demanding and
      // not recommended");
      int total_num_points = 0;
