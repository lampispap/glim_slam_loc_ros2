...
struct OdometryEstimationCPUParams : public OdometryEstimationIMUParams {
  ...
  double target_downsampling_rate;  ///< Downsampling rate for points to be
                                    ///< inserted into the target

  double ivox_resolution;  ///< iVox resolution (for GICP)
  double ivox_min_dist;    ///< Minimum distance between points in an iVox cell
                           ///< (for GICP)
  ...
  double vgicp_voxelmap_scaling_factor;  ///< Multi-resolution voxelmap scaling
                                         ///< factor (for VGICP)
};
...
class OdometryEstimationCPU : public OdometryEstimationIMU {
  ...
  OdometryEstimationCPU(const OdometryEstimationCPUParams& params =
                                OdometryEstimationCPUParams());
  ...
  gtsam::NonlinearFactorGraph create_wheel_odometry_factor(
  ...
  std::vector<std::shared_ptr<gtsam_points::GaussianVoxelMapCPU>>
          target_voxelmaps;                         ///< VGICP target voxelmap
  std::shared_ptr<gtsam_points::iVox> target_ivox;  ///< GICP target iVox
  EstimationFrame::ConstPtr
          target_ivox_frame;  ///< Target points (just for visualization)
};
