...
class CloudCovarianceEstimation {
   ...
   * @param neighbors Neighbor indices (must be N * k, where N is the number of
   * points)
   * @param normals   [output] Estimated normals
   * @param covs      [output] Estimated covariances
   */
  void estimate(const std::vector<Eigen::Vector4d>& points,
   ...
   * @brief Estimate point normals and covariances
   * @param points      Input points
   * @param neighbors   Neighbor indices (must be N * m, where N is the number
   * of points)
   ...
   */
  Eigen::Matrix4d regularize(const Eigen::Matrix4d& cov,
                             Eigen::Vector3d* eigenvalues = nullptr,
                             Eigen::Matrix3d* eigenvectors = nullptr) const;
