...
gtsam::NonlinearFactorGraph GlobalMapping::create_gps_factor(const int current, gtsam::Values& new_values) {
  ...
  double closest_gps_timestamp = -1;
  double best_distance = -1;
  const int center = submaps[current]->odom_frames.size() / 2;
...
void GlobalMapping::insert_submap(const SubMap::Ptr& submap) {
  ...
  if (params.enable_gps)
    new_factors->add(create_gps_factor(current, *new_values));
...
bool GlobalMapping::load(const std::string& path) {
  ...
  std::string token;
  int num_submaps, num_all_frames, num_matching_cost_factors, num_gps_factors;
  ...
  logger->info("creating gps factors");
  for (const auto& factor : gps_factors) {
    const auto index = std::get<0>(factor);
    const auto meas_x = std::get<1>(factor);
    const auto meas_y = std::get<2>(factor);
    const auto meas_z = std::get<3>(factor);
    const auto sigma_x = std::get<4>(factor);
