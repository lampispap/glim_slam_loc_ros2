...
GlobalMappingPoseGraphParams::GlobalMappingPoseGraphParams() {
  ...
  registration_type = config.param<std::string>("global_mapping",
                                                "registration_type", "GICP");

  min_travel_dist =
          config.param<double>("global_mapping", "min_travel_dist", 100.0);
  max_neighbor_dist =
          config.param<double>("global_mapping", "max_neighbor_dist", 10.0);
  min_inliear_fraction =
          config.param<double>("global_mapping", "min_inliear_fraction", 0.5);

  subsample_target =
          config.param<int>("global_mapping", "subsample_target", 10000);
  subsample_rate =
          config.param<double>("global_mapping", "subsample_rate", 0.1);
  gicp_max_correspondence_dist = config.param<double>(
          "global_mapping", "gicp_max_correspondence_dist", 2.0);
  vgicp_voxel_resolution =
          config.param<double>("global_mapping", "vgicp_voxel_resolution", 2.0);

  odom_factor_stddev =
          config.param<double>("global_mapping", "odom_factor_stddev", 1e-3);
  loop_factor_stddev =
          config.param<double>("global_mapping", "loop_factor_stddev", 0.1);
  loop_factor_robust_width = config.param<double>(
          "global_mapping", "loop_factor_robust_width", 1.0);

  loop_candidate_buffer_size = config.param<int>(
          "global_mapping", "loop_candidate_buffer_size", 100);
  loop_candidate_eval_per_thread = config.param<int>(
          "global_mapping", "loop_candidate_eval_per_thread", 2);
...
GlobalMappingPoseGraph::GlobalMappingPoseGraph(
        const GlobalMappingPoseGraphParams& params)
    : params(params) {
...
void GlobalMappingPoseGraph::insert_submap(int current,
  ...
  if (params.subsample_target > 0) {
    target->subsampled = gtsam_points::random_sampling(
            submap->frame,
            static_cast<double>(params.subsample_target) /
                    submap->frame->size(),
            mt);
  } else {
    if (params.subsample_rate > 0.99) {
      ...
      target->subsampled = gtsam_points::random_sampling(
              submap->frame, params.subsample_rate, mt);
    }
  }

  // Create nearest neighbor search
  if (params.registration_type == "GICP") {
    target->tree = std::make_shared<gtsam_points::KdTree>(
            submap->frame->points, submap->frame->size());
  } else if (params.registration_type == "VGICP") {
    target->voxels = std::make_shared<gtsam_points::GaussianVoxelMapCPU>(
            params.vgicp_voxel_resolution);
  ...
  if (current == 0) {
    ...
    const double displacement =
            (submaps[current - 1]->T_world_origin.translation() -
             submaps[current]->T_world_origin.translation())
                    .norm();
...
GlobalMappingPoseGraph::create_odometry_factors(int current) const {
  ...
  const int last = current - 1;
  const gtsam::Pose3 T_last_current = gtsam::Pose3(
          (submaps[last]->origin_frame()->T_world_sensor().inverse() *
           submaps[current]->origin_frame()->T_world_sensor())
                  .matrix());
  factors->emplace_shared<gtsam::BetweenFactor<gtsam::Pose3>>(
          X(last), X(current), T_last_current,
          gtsam::noiseModel::Isotropic::Sigma(6, params.odom_factor_stddev));
...
void GlobalMappingPoseGraph::find_loop_candidates(int current) {
  ...
  for (int i = 0; i < submaps.size() - 1; i++) {
    ...
    const double direct_dist = (submaps[current]->T_world_origin.translation() -
                                submaps[i]->T_world_origin.translation())
                                       .norm();
    if (direct_dist > params.max_neighbor_dist) {
      ...
      if (i != 0 && direct_dist > params.max_neighbor_dist * 2) {
        ...
        const double travel_dist_avg = (submap_targets[i]->travel_dist -
                                        submap_targets[left]->travel_dist) /
                                       std::max(i - left, 1);
    ...
    const double travel_dist = submap_targets[current]->travel_dist -
                               submap_targets[i]->travel_dist;
    ...
    const Eigen::Isometry3d T_target_source =
            submaps[i]->T_world_origin.inverse() *
            submaps[current]->T_world_origin;
    new_candidates.emplace_back(LoopCandidate{
            submap_targets[i], submap_targets[current], T_target_source});
  }
...
std::shared_ptr<gtsam::NonlinearFactorGraph>
GlobalMappingPoseGraph::collect_detected_loops() {
  auto factors = gtsam::make_shared<gtsam::NonlinearFactorGraph>();
...
void GlobalMappingPoseGraph::loop_detection_task() {
  ...
  while (!kill_switch) {
    ...
    candidates_buffer.insert(candidates_buffer.end(), new_candidates.begin(),
                             new_candidates.end());
    ...
    const int eval_count =
            params.loop_candidate_eval_per_thread * params.num_threads;
    ...
    if (candidates_buffer.size() < eval_count) {
      ...
      candidates.assign(candidates_buffer.begin(),
                        candidates_buffer.begin() + eval_count);
      candidates_buffer.erase(candidates_buffer.begin(),
                              candidates_buffer.begin() + eval_count);
    }
...
#pragma omp parallel for num_threads(params.num_threads) schedule(dynamic)
    for (int i = 0; i < candidates.size(); i++) {
      ...
      gtsam::Values values;
      values.insert(0,
                    gtsam::Pose3(candidates[i].init_T_target_source.matrix()));

      double error, inlier_fraction;

      if (params.registration_type == "GICP") {
        auto factor = gtsam::make_shared<gtsam_points::IntegratedGICPFactor>(
                gtsam::Pose3(), 0, candidate.target->submap->frame,
                candidate.source->subsampled, candidate.target->tree);
        factor->set_max_correspondence_distance(
                params.gicp_max_correspondence_dist);
        ...
        auto factor = gtsam::make_shared<gtsam_points::IntegratedVGICPFactor>(
                gtsam::Pose3(), 0, candidate.target->voxels,
                candidate.source->subsampled);
      ...
      logger->debug("target={}, source={}, error={}, inlier_fraction={}",
                    target->submap->id, source->submap->id, error,
                    inlier_fraction);
    ...
    for (int i = 0; i < candidates.size(); i++) {
      ...
      gtsam::SharedNoiseModel noise_model =
              gtsam::noiseModel::Isotropic::Sigma(6, params.loop_factor_stddev);
      noise_model = gtsam::noiseModel::Robust::Create(
              gtsam::noiseModel::mEstimator::Huber::Create(
                      params.loop_factor_robust_width),
              noise_model);
      factors.emplace_back(
              gtsam::make_shared<gtsam::BetweenFactor<gtsam::Pose3>>(
                      X(candidates[i].target->submap->id),
                      X(candidates[i].source->submap->id), T_target_source[i],
                      noise_model));
    }
