...
SubMapping::SubMapping(const SubMappingParams& params) : params(params) {
  ...
  T_ned_odom = Eigen::Isometry3d::Identity();
...
void SubMapping::insert_frame(const EstimationFrame::ConstPtr& odom_frame_) {
  ...
  Eigen::Isometry3d initial_pose = T_ned_odom * odom_frame->T_world_sensor();
  ...
  if (params.enable_gps) {
    graph->add(create_gps_factor(current, *values));
  }
  ...
  if (new_submap) {
    ...
    logger->info("Update T_odom_ned: {}", convert_to_string(T_ned_odom));
  }
}
...
void SubMapping::insert_gps(double stamp, const Eigen::Vector4d& lat_lon_alt) {
  ...
  if (!geoconverter_initialized) {
    ...
    Callbacks::on_set_gps_origin(lat_lon_alt[0], lat_lon_alt[1]);
  }
  ...
  if (current > 0) {
    while (!gps_queue.empty())
    {
      if (gps_queue.front().first <= odom_frames[current]->stamp) {
        ...
        gps_queue.pop_front();
        if (gps_data.second[3] > params.gps_max_hdop) {
          logger->info("GPS HDOP too large: {} ignore", gps_data.second[3]);
          continue;
        }
...
std::vector<SubMap::Ptr> SubMapping::submit_end_of_sequence() {
  std::vector<SubMap::Ptr> submaps;
  if (!keyframes.empty() && !odom_frames.empty()) {
