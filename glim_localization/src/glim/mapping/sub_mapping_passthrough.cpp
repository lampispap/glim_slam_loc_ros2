...
SubMappingPassthroughParams::SubMappingPassthroughParams() {
  ...
  keyframe_update_interval_rot = config.param<double>(
          "sub_mapping", "keyframe_update_interval_rot", 0.01);
  keyframe_update_interval_trans = config.param<double>(
          "sub_mapping", "keyframe_update_interval_trans", 0.1);

  max_num_keyframes = config.param<int>("sub_mapping", "max_num_keyframes", 50);
  max_num_keyframes = max_num_keyframes < 0 ? std::numeric_limits<int>::max()
                                            : max_num_keyframes;

  max_num_voxels = config.param<int>("sub_mapping", "max_num_voxels", 50000);
  max_num_voxels =
          max_num_voxels < 0 ? std::numeric_limits<int>::max() : max_num_voxels;

  adaptive_max_num_voxels =
          config.param<double>("sub_mapping", "adaptive_max_num_voxels", 0.5);
  adaptive_max_num_voxels = adaptive_max_num_voxels < 0
                                    ? std::numeric_limits<double>::max()
                                    : adaptive_max_num_voxels;

  submap_target_num_points =
          config.param<int>("sub_mapping", "submap_target_num_points", 40000);
  submap_voxel_resolution =
          config.param<double>("sub_mapping", "submap_voxel_resolution", 0.5);
  min_dist_in_voxel =
          config.param<double>("sub_mapping", "min_dist_in_voxel", 0.1);
  max_num_points_in_voxel =
          config.param<int>("sub_mapping", "max_num_points_in_voxel", 100);
}

SubMappingPassthroughParams::~SubMappingPassthroughParams() {}

SubMappingPassthrough::SubMappingPassthrough(
        const SubMappingPassthroughParams& params)
    : params(params) {
  submap_count = 0;
  ...
  voxelmap->voxel_insertion_setting().set_max_num_points_in_cell(
          params.max_num_points_in_voxel);
  voxelmap->voxel_insertion_setting().set_min_dist_in_cell(
          params.min_dist_in_voxel);
}

SubMappingPassthrough::~SubMappingPassthrough() {}

void SubMappingPassthrough::insert_frame(
        const EstimationFrame::ConstPtr& odom_frame) {
  logger->trace("insert_frame frame_id={} stamp={}", odom_frame->id,
                odom_frame->stamp);
  ...
  if (!keyframes.empty()) {
    ...
    insert_as_keyframe = dt > params.keyframe_update_interval_trans ||
                         dr > params.keyframe_update_interval_rot;
  ...
  if (insert_as_keyframe) {
    ...
    // TODO: May need to consider accumulating points in a local frame to avoid
    // voxel coord overflow
    auto transformed = gtsam_points::transform(odom_frame->frame,
                                               odom_frame->T_world_sensor());
...
SubMap::Ptr SubMappingPassthrough::create_submap(bool force_create) const {
  ...
  const auto check_adaptive_num_voxels = [&] {
    ...
    return voxelmap->num_voxels() <
           init_num_voxels * params.adaptive_max_num_voxels;
  };

  // Check if there are enough frames to compose a submap
  if (!force_create && keyframes.size() < params.max_num_keyframes &&
      voxelmap->num_voxels() < params.max_num_voxels &&
      check_adaptive_num_voxels()) {
    return nullptr;
  }
  ...
  submap->T_origin_endpoint_L = submap->T_world_origin.inverse() *
                                odom_frames.front()->T_world_sensor();
  submap->T_origin_endpoint_R = submap->T_world_origin.inverse() *
                                odom_frames.back()->T_world_sensor();
  ...
  submap->frame =
          gtsam_points::transform(merged, submap->T_world_origin.inverse());
