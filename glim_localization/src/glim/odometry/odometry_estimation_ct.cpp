...
OdometryEstimationCTParams::OdometryEstimationCTParams() {
  ...
  max_correspondence_distance = config.param<double>(
          "odometry_estimation", "max_correspondence_distance", 1.0);

  ivox_resolution =
          config.param<double>("odometry_estimation", "ivox_resolution", 1.0);
  ivox_min_points_dist = config.param<double>("odometry_estimation",
                                              "ivox_min_points_dist", 0.05);
  ivox_lru_thresh =
          config.param<int>("odometry_estimation", "ivox_lru_thresh", 30);

  location_consistency_inf_scale = config.param<double>(
          "odometry_estimation", "location_consistency_inf_scale", 1e-3);
  constant_velocity_inf_scale = config.param<double>(
          "odometry_estimation", "constant_velocity_inf_scale", 1e-3);
  lm_max_iterations =
          config.param<int>("odometry_estimation", "lm_max_iterations", 8);
...
OdometryEstimationCT::OdometryEstimationCT(
        const OdometryEstimationCTParams& params)
    : params(params) {
...
EstimationFrame::ConstPtr OdometryEstimationCT::insert_frame(
  ...
  gtsam_points::PointCloudCPU::Ptr frame_cpu(
          new gtsam_points::PointCloudCPU(raw_frame->points));
  frame_cpu->add_times(raw_frame->times);

  covariance_estimation->estimate(raw_frame->points, raw_frame->neighbors,
                                  frame_cpu->normals_storage,
                                  frame_cpu->covs_storage);
  ...
  // note: X(i) and Y(i) respectively represent the sensor poses at the scan
  // beginning and ending of i-th frame
  gtsam::FixedLagSmootherKeyTimestampMap new_stamps;
  gtsam::Values new_values;
  gtsam::NonlinearFactorGraph new_factors;

  if (current == 0) {
    new_frame->set_T_world_sensor(FrameID::LIDAR,
    ...
    if (current >= 2) {
      if (!frames[last] || !frames[last - 1]) {
        logger->warn(
                ...
                fmt::ptr(frames[last]), fmt::ptr(frames[last - 1]));
        logger->warn(
                "there might be a large time gap between point cloud frames");
      } else {
        const double delta_time =
                (frames[last]->stamp +
                 frames[last]->frame->times[frames[last]->frame->size() - 1]) -
                frames[last - 1]->stamp;
        const gtsam::Pose3 delta_pose =
                smoother->calculateEstimate<gtsam::Pose3>(X(last - 1))
                        .inverse() *
                smoother->calculateEstimate<gtsam::Pose3>(Y(last));
        last_twist = 0.85 * gtsam::Pose3::Logmap(delta_pose) / delta_time;
      }
    }

    const auto& last_frame = frames[last];
    const double last_time_begin =
            last_frame->stamp + last_frame->frame->times[0];
    const double last_time_end =
            last_frame->stamp +
            last_frame->frame->times[last_frame->frame->size() - 1];
    const gtsam::Pose3 last_T_world_lidar_begin =
            smoother->calculateEstimate<gtsam::Pose3>(X(last));
    const gtsam::Pose3 last_T_world_lidar_end =
            smoother->calculateEstimate<gtsam::Pose3>(Y(last));

    // Initial guess
    const double current_time_begin =
            new_frame->stamp + new_frame->frame->times[0];
    const double current_time_end =
            new_frame->stamp +
            new_frame->frame->times[new_frame->frame->size() - 1];
    const gtsam::Pose3 predicted_T_world_lidar_begin =
            last_T_world_lidar_end *
            gtsam::Pose3::Expmap(last_twist *
                                 (current_time_begin - last_time_end));
    const gtsam::Pose3 predicted_T_world_lidar_end =
            predicted_T_world_lidar_begin *
            gtsam::Pose3::Expmap(last_twist *
                                 (current_time_end - current_time_begin));
    ...
    auto factor = gtsam::make_shared<gtsam_points::IntegratedCT_GICPFactor_<
            gtsam_points::iVox, gtsam_points::PointCloud>>(
            X(current), Y(current), target_ivox, new_frame->frame, target_ivox);
    ...
    graph.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(
            X(current), last_T_world_lidar_end,
            gtsam::noiseModel::Isotropic::Precision(
                    6, params.location_consistency_inf_scale));
    graph.emplace_shared<gtsam::BetweenFactor<gtsam::Pose3>>(
            X(current), Y(current), gtsam::Pose3(),
            gtsam::noiseModel::Isotropic::Precision(
                    6, params.constant_velocity_inf_scale));
    ...
    const gtsam::Pose3 T_world_lidar_begin =
            values.at<gtsam::Pose3>(X(current));
    const gtsam::Pose3 T_world_lidar_end = values.at<gtsam::Pose3>(Y(current));

    new_frame->v_world_imu = (T_world_lidar_begin.translation() -
                              last_T_world_lidar_begin.translation()) /
                             (current_time_begin - last_time_begin);
    new_frame->set_T_world_sensor(
            FrameID::LIDAR,
    ...
    auto deskewed_covs = covariance_estimation->estimate(deskewed_points,
                                                         raw_frame->neighbors);
    ...
    new_factors.emplace_shared<gtsam::BetweenFactor<gtsam::Pose3>>(
            ...
            gtsam::noiseModel::Isotropic::Precision(6, 1e3));
    new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(
            X(current), T_world_lidar_begin,
            gtsam::noiseModel::Isotropic::Precision(6, 1e3));
    new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(
            Y(current), T_world_lidar_end,
            gtsam::noiseModel::Isotropic::Precision(6, 1e3));
  }
  ...
  for (int i = 0; i < transformed->size(); i++) {
    ...
    transformed->covs[i] = new_frame->T_world_sensor().matrix() *
                           new_frame->frame->covs[i] *
                           new_frame->T_world_sensor().matrix().transpose();
  }
  ...
  for (int i = marginalized_cursor; i < frames.size(); i++) {
    try {
      Eigen::Isometry3d T_world_lidar = Eigen::Isometry3d(
