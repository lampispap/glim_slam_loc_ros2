...
OdometryEstimationGPUParams::OdometryEstimationGPUParams()
  ...
  voxel_resolution =
          config.param<double>("odometry_estimation", "voxel_resolution", 0.5);
  voxelmap_levels =
          config.param<int>("odometry_estimation", "voxelmap_levels", 2);
  voxelmap_scaling_factor = config.param<double>(
          "odometry_estimation", "voxelmap_scaling_factor", 2.0);

  max_num_keyframes =
          config.param<int>("odometry_estimation", "max_num_keyframes", 10);
  full_connection_window_size = config.param<int>(
          "odometry_estimation", "full_connection_window_size", 3);

  const std::string strategy = config.param<std::string>(
          "odometry_estimation", "keyframe_update_strategy", "OVERLAP");
  if (strategy == "OVERLAP") {
  ...
  keyframe_min_overlap = config.param<double>("odometry_estimation",
                                              "keyframe_min_overlap", 0.1);
  keyframe_max_overlap = config.param<double>("odometry_estimation",
                                              "keyframe_max_overlap", 0.9);
  keyframe_delta_trans = config.param<double>("odometry_estimation",
                                              "keyframe_delta_trans", 1.0);
  keyframe_delta_rot = config.param<double>("odometry_estimation",
                                            "keyframe_delta_rot", 0.25);
  keyframe_entropy_thresh = config.param<double>(
          "odometry_estimation", "keyframe_entropy_thresh", 0.99);
}

OdometryEstimationGPUParams::~OdometryEstimationGPUParams() {}

OdometryEstimationGPU::OdometryEstimationGPU(
        const OdometryEstimationGPUParams& params)
    : OdometryEstimationIMU(
              std::make_unique<OdometryEstimationGPUParams>(params)) {
  ...
  stream_buffer_roundrobin.reset(
          new gtsam_points::StreamTempBufferRoundRobin());
}
...
void OdometryEstimationGPU::create_frame(EstimationFrame::Ptr& new_frame) {
  const auto params =
          static_cast<OdometryEstimationGPUParams*>(this->params.get());
  ...
  for (int i = 0; i < params->voxelmap_levels; i++) {
    ...
    const double resolution = params->voxel_resolution *
                              std::pow(params->voxelmap_scaling_factor, i);
...
void OdometryEstimationGPU::update_frames(
        const int current, const gtsam::NonlinearFactorGraph& new_factors) {
...
gtsam::NonlinearFactorGraph OdometryEstimationGPU::create_factors(
        const int current,
  ...
  const auto create_binary_factor =
          [this](gtsam::NonlinearFactorGraph& factors, gtsam::Key target_key,
            ...
            for (const auto& voxelmap : target->voxelmaps) {
              auto factor = gtsam::make_shared<
                      ...
                      target_key, source_key, voxelmap, source->frame, stream,
                      buffer);
  ...
  const auto create_unary_factor =
          [this](gtsam::NonlinearFactorGraph& factors,
            ...
            for (const auto& voxelmap : target->voxelmaps) {
              auto factor = gtsam::make_shared<
                      gtsam_points::IntegratedVGICPFactorGPU>(
                      fixed_target_pose, source_key, voxelmap, source->frame,
                      stream, buffer);
              factor->set_enable_surface_validation(true);
              factors.add(factor);
            }
  ...
  for (int target = current - params->full_connection_window_size;
       target < current; target++) {
    if (target < 0) {
      continue;
    }

    create_binary_factor(factors, X(target), X(current), frames[target],
                         frames[current]);
  }
  ...
  for (const auto& keyframe : keyframes) {
    ...
    if (span > params->smoother_lag - 0.1) {
      ...
      create_unary_factor(factors, key_T_world_imu, X(current), keyframe,
                          frames[current]);
    } else {
 ...
 * @ref   Koide et al., "Globally Consistent and Tightly Coupled 3D LiDAR
 * Inertial Mapping", ICRA2022
 */
void OdometryEstimationGPU::update_keyframes_overlap(int current) {
  ...
  std::vector<gtsam_points::GaussianVoxelMap::ConstPtr> keyframes_(
          keyframes.size());
  std::vector<Eigen::Isometry3d> delta_from_keyframes(keyframes.size());
  for (int i = 0; i < keyframes.size(); i++) {
    keyframes_[i] = keyframes[i]->voxelmaps.back();
    delta_from_keyframes[i] =
            keyframes[i]->T_world_imu.inverse() * frames[current]->T_world_imu;
  }

  const double overlap = gtsam_points::overlap_gpu(
          keyframes_, frames[current]->frame, delta_from_keyframes, *stream);
  ...
  for (int i = 0; i < keyframes.size(); i++) {
    const Eigen::Isometry3d delta =
            keyframes[i]->T_world_imu.inverse() * new_keyframe->T_world_imu;
    const double overlap =
            gtsam_points::overlap_gpu(keyframes[i]->voxelmaps.back(),
                                      new_keyframe->frame, delta, *stream);
  ...
  for (int i = 0; i < keyframes.size() - 1; i++) {
    ...
    const double overlap_latest = gtsam_points::overlap_gpu(
            keyframe->voxelmaps.back(), new_keyframe->frame,
    ...
    for (int j = 0; j < keyframes.size() - 1; j++) {
      ...
      delta_from_others.push_back(other->T_world_imu.inverse() *
                                  keyframe->T_world_imu);
    }

    const double overlap_others = gtsam_points::overlap_gpu(
            other_keyframes, keyframe->frame, delta_from_others, *stream);
...
void OdometryEstimationGPU::update_keyframes_displacement(int current) {
  ...
  const Eigen::Isometry3d delta_from_last =
          keyframes.back()->T_world_imu.inverse() *
  ...
  if (delta_trans < params->keyframe_delta_trans &&
      delta_rot < params->keyframe_delta_rot) {
    return;
  }
  ...
  for (int i = leave_window; i < keyframes.size() - 1; i++) {
    ...
    for (int j = 0; j < keyframes.size() - 1; j++) {
      ...
      const double dist = (keyframes[i]->T_world_imu.translation() -
                           keyframes[j]->T_world_imu.translation())
                                  .norm();
      sum_inv_dist += 1.0 / (dist + eps);
    }

    const double d0 = (keyframes[i]->T_world_imu.translation() -
                       new_keyframe->T_world_imu.translation())
                              .norm();
 ...
 * @ref   Kuo et al., "Redesigning SLAM for Arbitrary Multi-Camera Systems",
 * ICRA2020
 */
void OdometryEstimationGPU::update_keyframes_entropy(
        const gtsam::NonlinearFactorGraph& matching_cost_factors, int current) {
  const auto params =
          static_cast<OdometryEstimationGPUParams*>(this->params.get());
  ...
  for (const auto& factor : matching_cost_factors) {
    bool valid = std::all_of(
            factor->keys().begin(), factor->keys().end(),
            [&](const gtsam::Key key) { return values.exists(key); });
    if (!valid) {
      continue;
    }
  ...
  entropy_num_frames++;
  entropy_running_average +=
          (negative_entropy - entropy_running_average) / entropy_num_frames;

  if (!keyframes.empty() &&
      negative_entropy >
              entropy_running_average * params->keyframe_entropy_thresh) {
    return;
  }
