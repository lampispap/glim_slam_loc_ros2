...
void OdometryEstimationIMU::insert_gps(double stamp, const Eigen::Vector4d& lat_lon_alt) {
...
EstimationFrame::ConstPtr OdometryEstimationIMU::insert_frame(const PreprocessedFrame::Ptr& raw_frame, std::vector<EstimationFrame::ConstPtr>& marginalized_frames) {
  ...
  if (frames.empty()) {
    ...
    if (params->enable_gps) {
      new_factors.emplace_shared<gtsam_points::LinearDampingFactor>(X(0), 6, 1.0);
      // new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(X(0), gtsam::Pose3(new_frame->T_world_imu.matrix()),
...
gtsam::NonlinearFactorGraph OdometryEstimationIMU::create_gps_factor(const int current, gtsam::Values& new_values, gtsam::FixedLagSmootherKeyTimestampMap & new_stamps) {
    ...
    while (!gps_queue.empty())
    {
      if (gps_queue.front().first <= frames[current]->stamp) {
        ...
        if (gps_data.first > frames[last]->stamp) {
          // initial guess
          double weight =  (gps_data.first - frames[last]->stamp) / (frames[current]->stamp - frames[last]->stamp);
          ...
          auto interpolate_nose = gtsam::noiseModel::Isotropic::Precision(6, 1e3);
          new_values.insert(G(gps_data_id), gps_pose);
          new_stamps[G(gps_data_id)] = gps_data.first;
          factor.add(gtsam_points::Pose3InterpolationFactor(X(last), X(current), G(gps_data_id), weight, interpolate_nose));
          ...
          auto gps_noise = gtsam::noiseModel::Diagonal::Sigmas(
            gtsam::Vector3(gps_data.second[3] / params->gps_noise, gps_data.second[3] / params->gps_noise, 1e2));
          factor.add(gtsam::GPSFactor(G(gps_data_id),
            gtsam::Point3(gps_data.second[0], gps_data.second[1], gps_pose.translation().z()), gps_noise));
      ...
      //     // factor.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(X(current), gtsam::Pose3(gps_pose.matrix()), gps_noise);
      //     // logger->info("Insert GPS factor {} for frames: {}", gps_data_id, current);
      //   }
      // }
      else {
        break;
