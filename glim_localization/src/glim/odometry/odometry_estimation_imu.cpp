...
using gtsam::symbol_shorthand::G;  // GPS pose       (T_world_imu)
...
OdometryEstimationIMUParams::OdometryEstimationIMUParams() {
  ...
  T_lidar_imu = sensor_config.param<Eigen::Isometry3d>(
          "sensors", "T_lidar_imu", Eigen::Isometry3d::Identity());
  T_robot_imu = sensor_config.param<Eigen::Isometry3d>(
          "sensors", "T_robot_imu", Eigen::Isometry3d::Identity());
  imu_bias_noise =
          sensor_config.param<double>("sensors", "imu_bias_noise", 1e-3);
  ...
  fix_imu_bias =
          config.param<bool>("odometry_estimation", "fix_imu_bias", false);

  initialization_mode = config.param<std::string>(
          "odometry_estimation", "initialization_mode", "LOOSE");
  const auto init_T_world_imu = config.param<Eigen::Isometry3d>(
          "odometry_estimation", "init_T_world_imu");
  const auto init_v_world_imu = config.param<Eigen::Vector3d>(
          "odometry_estimation", "init_v_world_imu");
  this->estimate_init_state = !init_T_world_imu && !init_v_world_imu;
  this->init_T_world_imu =
          init_T_world_imu.value_or(Eigen::Isometry3d::Identity());
  this->init_v_world_imu = init_v_world_imu.value_or(Eigen::Vector3d::Zero());
  this->init_pose_damping_scale = config.param<double>(
          "odometry_estimation", "init_pose_damping_scale", 1e10);

  smoother_lag =
          config.param<double>("odometry_estimation", "smoother_lag", 5.0);
  use_isam2_dogleg =
          config.param<bool>("odometry_estimation", "use_isam2_dogleg", false);
  isam2_relinearize_skip =
          config.param<int>("odometry_estimation", "isam2_relinearize_skip", 1);
  isam2_relinearize_thresh = config.param<double>(
          "odometry_estimation", "isam2_relinearize_thresh", 0.1);

  save_imu_rate_trajectory = config.param<bool>(
          "odometry_estimation", "save_imu_rate_trajectory", false);

  num_threads = config.param<int>("odometry_estimation", "num_threads", 4);
  num_smoother_update_threads = 1;

  wheel_radius =
          config.param<double>("odometry_estimation", "wheel_radius", 0.081);
  ...
  differential_drive_model_noise = config.param<double>(
          "odometry_estimation", "differential_drive_model_noise", 1e6);
  wheel_odometry_noise = config.param<double>("odometry_estimation",
                                              "wheel_odometry_noise", 1e8);

  enable_gps = config.param<bool>("odometry_estimation", "enable_gps", false);
  gps_noise = config.param<double>("odometry_estimation", "gps_noise", 1e6);
}

OdometryEstimationIMUParams::~OdometryEstimationIMUParams() {}

OdometryEstimationIMU::OdometryEstimationIMU(
        std::unique_ptr<OdometryEstimationIMUParams>&& params_)
  ...
  if (!params->estimate_init_state || params->initialization_mode == "NAIVE") {
    auto init_estimation = new NaiveInitialStateEstimation(params->T_lidar_imu,
                                                           params->imu_bias);
    if (!params->estimate_init_state) {
      init_estimation->set_init_state(params->init_T_world_imu,
                                      params->init_v_world_imu);
    }
    this->init_estimation.reset(init_estimation);
  } else if (params->initialization_mode == "LOOSE") {
    auto init_estimation = new LooseInitialStateEstimation(params->T_lidar_imu,
                                                           params->imu_bias);
    this->init_estimation.reset(init_estimation);
  } else {
    logger->error("unknown initialization mode {}",
  ...
  covariance_estimation.reset(
          new CloudCovarianceEstimation(params->num_threads));
...
#ifdef GTSAM_USE_TBB
  tbb_task_arena = std::make_shared<tbb::task_arena>(
          params->num_smoother_update_threads);
#endif
}

OdometryEstimationIMU::~OdometryEstimationIMU() {}

void OdometryEstimationIMU::insert_imu(const double stamp,
...
void OdometryEstimationIMU::insert_gps(double stamp,
  ...
  Callbacks::on_gps_update(xyz[0], xyz[1], xyz[2]);
}

void OdometryEstimationIMU::insert_raw_odom(const double stamp,
                                            const double left_angular_vel,
                                            const double right_angular_vel) {
  if (params->use_wheel)
    odom_integration->insert_raw_odom(stamp, left_angular_vel,
                                      right_angular_vel);
}
...
EstimationFrame::ConstPtr OdometryEstimationIMU::insert_frame(
  ...
  if (raw_frame->size()) {
    logger->trace("insert_frame points={} times={} ~ {}", raw_frame->size(),
                  raw_frame->times.front(), raw_frame->times.back());
  } else {
  ...
  if (frames.empty()) {
    ...
    covariance_estimation->estimate(points_imu, raw_frame->neighbors, normals,
                                    covs);
    ...
    if (params->enable_gps) {
      new_factors.emplace_shared<gtsam_points::LinearDampingFactor>(X(0), 6,
                                                                    1.0);
      // new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(X(0),
      // gtsam::Pose3(new_frame->T_world_imu.matrix()),
      //   gtsam::noiseModel::Isotropic::Precision(6, 1e-10));
    ...
    if (params->enable_gps) {
      new_factors.add(create_gps_factor(current, new_values, new_stamps));
    }
  ...
  const auto last_T_world_imu =
          smoother->calculateEstimate<gtsam::Pose3>(X(last));
  const auto last_v_world_imu =
          smoother->calculateEstimate<gtsam::Vector3>(V(last));
  const auto last_imu_bias =
          smoother->calculateEstimate<gtsam::imuBias::ConstantBias>(B(last));
  ...
  const int imu_read_cursor = imu_integration->integrate_imu(
          last_stamp, raw_frame->stamp, last_imu_bias, &num_imu_integrated);
  ...
  const gtsam::NavState predicted_nav_world_imu =
          imu_integration->integrated_measurements().predict(last_nav_world_imu,
                                                             last_imu_bias);
  const gtsam::Pose3 predicted_T_world_imu = predicted_nav_world_imu.pose();
  const gtsam::Vector3 predicted_v_world_imu =
          predicted_nav_world_imu.velocity();
  ...
  if (params->fix_imu_bias) {
    new_factors.add(gtsam::PriorFactor<gtsam::imuBias::ConstantBias>(
            B(current), gtsam::imuBias::ConstantBias(params->imu_bias),
  ...
  if (num_imu_integrated >= 2) {
    ...
    logger->warn(
            "insufficient number of IMU data between LiDAR scans!! "
            "(odometry_estimation)");
    logger->warn("t_last={:.6f} t_current={:.6f} num_imu={}", last_stamp,
                 raw_frame->stamp, num_imu_integrated);
  ...
  imu_integration->integrate_imu(raw_frame->stamp, raw_frame->scan_end_time,
                                 predicted_nav_world_imu, last_imu_bias,
                                 pred_imu_times, pred_imu_poses);
  ...
  new_frame->T_world_lidar =
          Eigen::Isometry3d(predicted_T_world_imu.matrix()) * T_imu_lidar;
  ...
  if (params->save_imu_rate_trajectory) {
    ...
    for (int i = 0; i < pred_imu_times.size(); i++) {
      ...
      new_frame->imu_rate_trajectory.col(i) << pred_imu_times[i], trans,
              quat.x(), quat.y(), quat.z(), quat.w();
    }
  }

  // Deskew and tranform points into IMU frame
  auto deskewed = deskewing->deskew(T_imu_lidar, pred_imu_times, pred_imu_poses,
                                    raw_frame->stamp, raw_frame->times,
                                    raw_frame->points);
  ...
  covariance_estimation->estimate(deskewed, raw_frame->neighbors,
                                  deskewed_normals, deskewed_covs);
  ...
  while (marginalized_cursor < current) {
    ...
    marginalized_cursor++;
    // logger->info("marginalized_cursor: {}", marginalized_cursor);
  }
  ...
  std::vector<EstimationFrame::ConstPtr> active_frames(
          frames.begin() + marginalized_cursor, frames.end());
  Callbacks::on_update_frames(active_frames);
  logger->trace("frames updated");

  if (smoother->fallbackHappened()) {
    logger->warn("odometry estimation smoother fallback happened (time={})",
                 raw_frame->stamp);
  }
...
gtsam::NonlinearFactorGraph OdometryEstimationIMU::create_gps_factor(
        const int current,
        gtsam::Values& new_values,
        gtsam::FixedLagSmootherKeyTimestampMap& new_stamps) {
  ...
  if (current > 0) {
    while (!gps_queue.empty()) {
      if (gps_queue.front().first <= frames[current]->stamp) {
        auto gps_data = gps_queue.front();
        gps_queue.pop_front();

        if (gps_data.first > frames[last]->stamp) {
          // initial guess
          double weight = (gps_data.first - frames[last]->stamp) /
                          (frames[current]->stamp - frames[last]->stamp);
          auto gps_pose = gtsam_points::Pose3InterpolationFactor::initial_guess(
                  gtsam::Pose3(frames[last]->T_world_imu.matrix()),
                  gtsam::Pose3(frames[current]->T_world_imu.matrix()), weight);

          auto interpolate_nose =
                  gtsam::noiseModel::Isotropic::Precision(6, 1e3);
          new_values.insert(G(gps_data_id), gps_pose);
          ...
          auto gps_noise = gtsam::noiseModel::Diagonal::Sigmas(
                  gtsam::Vector3(gps_data.second[3] / params->gps_noise,
                                 gps_data.second[3] / params->gps_noise, 1e2));
          factor.add(gtsam::GPSFactor(
                  G(gps_data_id),
                  gtsam::Point3(gps_data.second[0], gps_data.second[1],
                                gps_pose.translation().z()),
                  gps_noise));
      ...
      // if (gps_queue.front().first <= frames[current]->stamp + 0.02) {
      //   auto gps_data = gps_queue.front();
      //   gps_queue.pop_front();

      //   if (gps_data.first >= frames[current]->stamp - 0.02) {
      //     auto gps_noise = gtsam::noiseModel::Diagonal::Sigmas(
      //       gtsam::Vector3(gps_data.second[3] / params->gps_noise,
      //       gps_data.second[3] / params->gps_noise, 1e2));

      //     auto gps_pose = frames[current]->T_world_imu;
      //     gps_pose.translation().x() = gps_data.second[0];
      //     gps_pose.translation().y() = gps_data.second[1];

      //     factor.add(gtsam::GPSFactor(X(current),
      //       gtsam::Point3(gps_data.second[0], gps_data.second[1],
      //       gps_pose.translation().z()), gps_noise));

      //     //
      //     factor.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(X(current),
      //     gtsam::Pose3(gps_pose.matrix()), gps_noise);
      //     // logger->info("Insert GPS factor {} for frames: {}", gps_data_id,
      //     current);
      //   }
      // }
      else {
...
OdometryEstimationIMU::get_remaining_frames() {
  // Perform a few optimization iterations at the end
  // for(int i=0; i<5; i++) {
  //   smoother->update();
  // }
  // OdometryEstimationIMU::update_frames(frames.size() - 1,
...
void OdometryEstimationIMU::update_frames(
        int current, const gtsam::NonlinearFactorGraph& new_factors) {
  logger->trace("update frames current={} marginalized_cursor={}", current,
                marginalized_cursor);

  for (int i = marginalized_cursor; i < frames.size(); i++) {
    try {
      Eigen::Isometry3d T_world_imu = Eigen::Isometry3d(
              smoother->calculateEstimate<gtsam::Pose3>(X(i)).matrix());
      Eigen::Vector3d v_world_imu =
              smoother->calculateEstimate<gtsam::Vector3>(V(i));
      Eigen::Matrix<double, 6, 1> imu_bias =
              smoother->calculateEstimate<gtsam::imuBias::ConstantBias>(B(i))
                      .vector();
...
void OdometryEstimationIMU::update_smoother(int count) {
  ...
  update_smoother(gtsam::NonlinearFactorGraph(), gtsam::Values(),
                  std::map<std::uint64_t, double>(), count - 1);
}
