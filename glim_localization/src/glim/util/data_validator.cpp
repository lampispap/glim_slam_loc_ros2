...
void DataValidator::timer_callback() {
  ...
  spdlog::debug("timer_callback done (elapsed={:.3f}[msec])",
...
void DataValidator::imu_callback(const double stamp,
  ...
  if (last_imu_stamp >= 0.0) {
    if (stamp < last_imu_stamp) {
      spdlog::warn(
              "IMU timestamp rewind detected!! (last={:.6f} current={:.6f})",
              last_imu_stamp, stamp);
    } else if (stamp - last_imu_stamp > 0.1) {
      spdlog::warn(
              "A large time gap between consecutive IMU data!! (last={:.6f} "
              "current={:.6f} diff={:.6f})",
              last_imu_stamp, stamp, stamp - last_imu_stamp);
    }
  }
...
void DataValidator::points_callback(
        const double stamp, const std::shared_ptr<RawPoints>& raw_points) {
  const auto t1 = std::chrono::high_resolution_clock::now();
  ...
  if (last_points_stamp >= 0.0) {
    if (stamp < last_points_stamp) {
      spdlog::warn(
              "Point cloud timestamp rewind detected!! (last={:.6f} "
              "current={:.6f})",
              last_points_stamp, stamp);
    } else if (stamp - last_points_stamp > 1.0) {
      spdlog::warn(
              "A large time gap between consecutive point clouds!! "
              "(last={:.6f} current={:.6f} diff={:.6f})",
              last_points_stamp, stamp, stamp - last_points_stamp);
    }
  }
  ...
  if (last_imu_stamp >= 0.0) {
    if (std::abs(last_imu_stamp - stamp) > 1.0) {
      spdlog::warn(
              "Too large time gap between points and IMU timestamps!! "
              "(IMU={:.6f} points={:.6f})",
              last_imu_stamp, stamp);
    }
  }
  ...
  spdlog::debug("points size={} stamp={:.6f} times={:.3f}~{:.3f}",
                raw_points->size(), raw_points->stamp, raw_points->times[0],
                raw_points->times[raw_points->size() - 1]);

  if (std::any_of(raw_points->times.begin(), raw_points->times.end(),
                  [](double t) { return t < 0.0; })) {
    spdlog::warn("A negative per-point timestamp found!! (t={:.6f})",
                 *std::min_element(raw_points->times.begin(),
                                   raw_points->times.end()));
  }

  if (std::any_of(raw_points->times.begin(), raw_points->times.end(),
                  [](double t) { return t > 1.0; })) {
    spdlog::warn("A large per-point timestamp found!! (t={:.6f})",
                 *std::max_element(raw_points->times.begin(),
                                   raw_points->times.end()));
  }

  for (int i = 0; i < raw_points->size(); i++) {
    if (!raw_points->points[i].array().isFinite().all()) {
      spdlog::warn("An invalid point found!! (pt={})",
                   convert_to_string(raw_points->points[i]));
      break;
    }
  }
  ...
  spdlog::debug("points_callback done (elapsed={:.3f}[msec])",
                std::chrono::duration_cast<std::chrono::nanoseconds>(
                        std::chrono::high_resolution_clock::now() - t1)
                                .count() /
                        1e6);
