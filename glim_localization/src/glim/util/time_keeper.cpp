...
PerPointTimeSettings::PerPointTimeSettings() {
  ...
  if (autoconf) {
    ...
    relative_time =
            config.param<bool>("sensors", "perpoint_relative_time", true);
    prefer_frame_time =
            config.param<bool>("sensors", "autoconf_prefer_frame_time", false);
    point_time_scale =
            config.param<double>("sensors", "perpoint_time_scale", 1.0);
  }
}
...
bool TimeKeeper::validate_imu_stamp(const double imu_stamp) {
  ...
  if (last_imu_stamp < 0.0) {
    ...
    spdlog::warn("current={:.6f} last={:.6f} diff={:.6f}", imu_stamp,
                 last_imu_stamp, imu_diff);
    return false;
  ...
  if (last_points_stamp > 0.0 && std::abs(points_diff) > 1.0) {
    ...
    spdlog::warn("points={:.6f} imu={:.6f} diff={:.6f}", last_points_stamp,
                 imu_stamp, points_diff);
  }
...
void TimeKeeper::process(const glim::RawPoints::Ptr& points) {
  ...
  if (points->points.size() != points->times.size()) {
    ...
    spdlog::error(
            "inconsistent # of points and # of timestamps found after time "
            "conversion!! |points|={} |times|={}",
            points->points.size(), points->times.size());
  }
  if (points->times.front() < 0.0 || points->times.back() < 0.0) {
    ...
    spdlog::error(
            ...
            "front={:.6f} back={:.6f}",
            points->times.front(), points->times.back());
  }
  ...
  if (points->stamp > 3000000000) {
    spdlog::warn(
            "frame timestamp is wrong (or GLIM has been used for over 40 "
            "years)!! frame={:.6f}",
            points->stamp);
  }
  ...
  if (last_points_stamp < 0.0) {
    ...
    spdlog::warn("current={:.6f} last={:.6f} diff={:.6f}", points->stamp,
                 last_points_stamp, time_diff);
...
void TimeKeeper::replace_points_stamp(const glim::RawPoints::Ptr& points) {
  ...
  if (points->times.empty()) {
    ...
    if (scan_duration > 0.0) {
      for (int i = 0; i < points->size(); i++) {
        points->times[i] =
                scan_duration * static_cast<double>(i) / points->size();
      }
    }
  ...
  const auto minmax_times =
          std::minmax_element(points->times.begin(), points->times.end());
  ...
  if (settings.autoconf) {
    ...
    if (min_time < 0.0) {
      ...
      if (settings.prefer_frame_time) {
        ...
        spdlog::warn(
                "add an offset to the frame timestamp to make per-point ones "
                "positive!!");
      }
    }
    ...
    if (max_time < 1.0) {
      ...
      spdlog::warn(
              ...
              min_time, max_time);
      spdlog::warn(
              "assume that point times are absolute and convert them to "
              "relative");

      if (min_time > 1e16) {
        spdlog::warn(
                "too large point timestamp (min={:.6f} max={:.6f} > 1e16) "
                "found!!",
                min_time, max_time);
        spdlog::warn(
                "maybe using a Livox LiDAR that use FLOAT64 nanosec per-point "
                "timestamps");
      ...
      if (settings.prefer_frame_time) {
        spdlog::warn(
                "frame timestamp will be prioritized over the first point "
                "timestamp!!");
      } else {
        spdlog::warn(
                "frame timestamp will be overwritten by the first point "
                "timestamp!!");
      }
    }
  }
...
double TimeKeeper::estimate_scan_duration(const double stamp) {
  ...
  double scan_duration =
          scan_duration_history[scan_duration_history.size() / 2];
