 ...
 */
void OdomIntegration::insert_raw_odom(const double stamp,
...
int OdomIntegration::integrate_odom(double start_time,
                                    double end_time,
                                    double& left_angle_displacement,
                                    double& right_angle_displacement) {
  int cursor = 0;
  ...
  if (dt > 0.0) {
    Eigen::Matrix<double, 3, 1> last_imu_frame =
            odom_itr == odom_queue.end() ? *(odom_itr - 1) : *odom_itr;
