...
LocalizationViewer::LocalizationViewer() : logger(create_module_logger("viewer")) {
...
bool LocalizationViewer::ok() const {
...
Eigen::Isometry3f LocalizationViewer::resolve_pose(const EstimationFrame::ConstPtr& frame) {
...
void LocalizationViewer::set_callbacks() {
  ...
  OdometryEstimationCallbacks::on_smoother_update.add([this](
    ...
    for (const auto& factor : new_factors) {
      ...
      if (factor->keys().size() == 1) {
        ...
        const auto matching_factor = std::dynamic_pointer_cast<gtsam_points::IntegratedMatchingCostFactor>(factor);
        if (matching_factor) {
...
#ifdef BUILD_GTSAM_POINTS_GPU
        const auto gpu_factor = std::dynamic_pointer_cast<gtsam_points::IntegratedVGICPFactorGPU>(factor);
        if (gpu_factor) {
          const auto l = [this, idx0](const gtsam::NonlinearFactor* factor) -> std::optional<FactorLine> {
            const auto found0 = odometry_poses.find(idx0);
            if (found0 == odometry_poses.end()) {
  ...
  LocalizationCallbacks::on_insert_localization_submap.add([this](const SubMap::ConstPtr& submap) {
    ...
    invoke([this, submap, T_world_origin] {
      ...
      auto cloud_buffer = std::make_shared<glk::PointCloudBuffer>(submap->frame->points, submap->frame->size());
      auto shader_setting = guik::Rainbow(T_world_origin->matrix().cast<float>()).add("info_values", info);
  ...
  LocalizationCallbacks::on_update_localization_submaps.add([this](const std::vector<SubMap::Ptr>& submaps) {
    if (submaps.size() == 0)
      return;
    ...
    invoke([this, latest_submap, submap_ids, submap_poses] {
      ...
      for (int i = 0; i < submap_ids.size(); i++) {
        ...
        auto drawable = viewer->find_drawable("localization_submap_" + std::to_string(submap_ids[i]));
        if (drawable.first) {
          drawable.first->add("model_matrix", submap_poses[i].matrix());
        }
        viewer->update_drawable("localization_submap_coord_" + std::to_string(submap_ids[i]), 
          glk::Primitives::sphere(), guik::VertexColor(submap_poses[i]));
        
        const Eigen::Vector4i info(static_cast<int>(PickType::FRAME), 0, 0, submap_ids[i]);

        viewer->update_drawable(
          "localization_submap_coord_" + std::to_string(submap_ids[i]),
          glk::Primitives::coordinate_system(),
          guik::VertexColor(submap_poses[i]).add("info_values", info));

        viewer->update_drawable(
          "sphere_" + std::to_string(submap_ids[i]),
          glk::Primitives::sphere(),
          guik::FlatColor(0.5f, 0.0f, 0.0f, 0.5f, submap_poses[i] * Eigen::UniformScaling<float>(0.5)).add("info_values", info).make_transparent());
        }
  ...
  // Update submaps callback
  LocalizationCallbacks::on_update_active_submaps.add([this](const std::map<int, SubMap::Ptr>& submaps) {
    if (submaps.size() == 0)
      return;
    ...
    invoke([this, latest_submap, submap_ids, submap_poses] {
      ...
      std::vector<Eigen::Vector3f> submap_positions(submap_ids.size() + num_map_submaps);
      ...
      for (size_t i = 0; i < submap_ids.size(); i++) {
        submap_positions[num_map_submaps + i] = submap_poses[i].translation();
      ...
      for (const auto& factor : global_between_factors) {
        if (factor.first >= submap_ids[0] && factor.first <= submap_ids.back() && 
           factor.second >= submap_ids[0] && factor.second <= submap_ids.back()) {
          indices.push_back(num_map_submaps + factor.first - submap_ids[0]);
          indices.push_back(num_map_submaps + factor.second - submap_ids[0]);
        }
      }
      ...
      for (const auto& factor : global_localization_factors) {
        if (factor.second >= submap_ids[0] && factor.second <= submap_ids.back()) {
          indices.push_back(factor.first);
          // logger->info("factor first: {}, {}", submap_positions[factor.first].x(), submap_positions[factor.first].y());
          indices.push_back(num_map_submaps + factor.second - submap_ids[0]);
        }
      }

      viewer->update_drawable("localization_factors", std::make_shared<glk::ThinLines>(submap_positions, indices), guik::FlatRed());
  ...
  GlobalMappingCallbacks::on_smoother_update.add([this](gtsam_points::ISAM2Ext& isam2, gtsam::NonlinearFactorGraph& new_factors, gtsam::Values& new_values) {
    ...
    for (const auto& factor : new_factors) {
      ...
      if (symbol0.chr() == 'm' && symbol1.chr() == 'x') {
        localization_factors.push_back(std::make_pair(symbol0.index(), symbol1.index()));
      }
    ...
    invoke([this, between_factors, localization_factors] { 
      ...
      global_localization_factors.insert(global_localization_factors.end(), localization_factors.begin(), localization_factors.end()); 
    });
  });
  ...
  if (ImGui::BeginPopupContextVoid("context menu")) {
    ...
    if (type == PickType::POINTS || type == PickType::FRAME) {
      if (ImGui::MenuItem("Relocalize here")) {
        logger->info("Relocalization at: {}, {}, {}", right_clicked_pos.x(), right_clicked_pos.y(), right_clicked_pos.z());
...
void LocalizationViewer::drawable_selection() {
  ...
  ImGui::Separator();
  // bool show_localization = show_localization_submaps || show_localization_factors;
  // if (ImGui::Checkbox("localization", &show_localization)) {
  //   show_localization_submaps = show_localization_factors = show_localization;
  // }
  ...
  ImGui::SameLine();
  std::vector<const char*> PoseIds = {"P0", "P1", "P2", "P3", "P4", "P5", "P6", "P7", "P8", "P9"};
  ImGui::SetNextItemWidth(92);
