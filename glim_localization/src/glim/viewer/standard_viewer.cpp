...
bool StandardViewer::ok() const { return !request_to_terminate; }
...
Eigen::Isometry3f StandardViewer::resolve_pose(
...
void StandardViewer::set_callbacks() {
  ...
  SubMappingCallbacks::on_gps_update.add([this](double px, double py, double pz,
                                                double hdop) {
    invoke([this, px, py, pz, hdop] {
      auto viewer = guik::LightViewer::instance();
      auto gps_pose = Eigen::Isometry3f::Identity();
      gps_pose.translation() = Eigen::Vector3f(px, py, pz);
      viewer->update_drawable(
              "gps_coord_" + std::to_string(gps_data_id),
              glk::Primitives::wire_sphere(),
              guik::FlatColor(MIN(1.0, hdop / 5.0), 1.0f, MIN(1.0, hdop / 5.0),
                              0.4f,
                              gps_pose * Eigen::UniformScaling<float>(0.5)));
      gps_data_id++;
    });
  });
          ...
          [this](const std::vector<SubMap::Ptr>& submaps) {
            ...
            invoke([this, latest_submap, submap_ids, submap_poses] {
              ...
              for (const auto& factor : global_between_factors) {
                if (factor.first < submap_ids.size() &&
                    factor.second < submap_ids.size()) {
