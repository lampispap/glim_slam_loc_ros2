...
InteractiveViewer::InteractiveViewer()
  ...
  enable_partial_rendering =
          config.param("interactive_viewer", "enable_partial_rendering", false);
  partial_rendering_budget =
          config.param("interactive_viewer", "partial_rendering_budget", 1024);

  z_range = config.param("interactive_viewer", "default_z_range",
  ...
  OdometryEstimationCallbacks::on_new_frame.add(
          std::bind(&InteractiveViewer::odometry_on_new_frame, this, _1));
  GlobalMappingCallbacks::on_insert_submap.add(
          std::bind(&InteractiveViewer::globalmap_on_insert_submap, this, _1));
  GlobalMappingCallbacks::on_update_submaps.add(
          std::bind(&InteractiveViewer::globalmap_on_update_submaps, this, _1));
  GlobalMappingCallbacks::on_smoother_update.add(std::bind(
          &InteractiveViewer::globalmap_on_smoother_update, this, _1, _2, _3));
  GlobalMappingCallbacks::on_smoother_update_result.add(
          std::bind(&InteractiveViewer::globalmap_on_smoother_update_result,
                    this, _1, _2));
  ...
  viewer->register_drawable_filter("filter", [this](const std::string& name) {
    const auto starts_with = [](const std::string& name,
      ...
      return name.size() < pattern.size()
                     ? false
                     : std::equal(pattern.begin(), pattern.end(), name.begin());
    };
...
void InteractiveViewer::drawable_selection() {
  ...
  do_update_viewer |=
          ImGui::DragFloat("coord scale", &coord_scale, 0.01f, 0.01f, 100.0f);
  do_update_viewer |=
          ImGui::DragFloat("sphere scale", &sphere_scale, 0.01f, 0.01f, 100.0f);
  ...
  if (ImGui::BeginPopupContextVoid("context menu")) {
    ...
    if (type == PickType::FRAME) {
      ...
      if (ImGui::MenuItem("Loop begin")) {
        manual_loop_close_modal->set_target(
                X(frame_id), submaps[frame_id]->frame, submap_poses[frame_id]);
      }
      if (ImGui::MenuItem("Loop end")) {
        manual_loop_close_modal->set_source(
                X(frame_id), submaps[frame_id]->frame, submap_poses[frame_id]);
      }
    }
...
void InteractiveViewer::run_modals() {
  ...
  factors.erase(std::remove(factors.begin(), factors.end(), nullptr),
                factors.end());
...
void InteractiveViewer::update_viewer() {
  ...
  for (int i = 0; i < submaps.size(); i++) {
    ...
    auto drawable =
            viewer->find_drawable("submap_" + std::to_string(submap->id));
    if (drawable.first) {
      drawable.first->add("model_matrix", submap_pose.matrix());
    } else {
      const Eigen::Vector4i info(static_cast<int>(PickType::POINTS), 0, 0,
                                 submap->id);
    ...
    const Eigen::Vector4i info(static_cast<int>(PickType::FRAME), 0, 0,
                               submap->id);

    viewer->update_drawable(
            "coord_" + std::to_string(submap->id),
            glk::Primitives::coordinate_system(),
            guik::VertexColor(submap_pose *
                              Eigen::UniformScaling<float>(coord_scale))
                    .add("info_values", info));

    viewer->update_drawable(
            "sphere_" + std::to_string(submap->id), glk::Primitives::sphere(),
            guik::FlatColor(
                    1.0f, 0.0f, 0.0f, 0.5f,
  ...
  const auto get_position = [this](const gtsam::Key key) -> Eigen::Vector3d {
    ...
    switch (symbol.chr()) {
      ...
      case 'e': {
        ...
        const auto& T_origin_endpoint = right ? submap->T_origin_endpoint_R
                                              : submap->T_origin_endpoint_L;
        const Eigen::Isometry3d T_world_endpoint =
                submap->T_world_origin * T_origin_endpoint;

        return T_world_endpoint.translation();
      }
      case 'g':
        return Eigen::Vector3d(global_gps_factors[symbol.index()].matrix());
    }
  ...
  for (const auto& factor : global_gps_factors) {
    auto point = factor.second;
    auto gps_pose = Eigen::Isometry3f::Identity();
    gps_pose.translation() = Eigen::Vector3f(point.x(), point.y(), point.z());
    viewer->update_drawable(
            "gps_coord_" + std::to_string(factor.first),
            glk::Primitives::wire_sphere(),
            guik::FlatColor(0.5, 1.0f, 1.0, 0.8f,
                            gps_pose * Eigen::UniformScaling<float>(0.25)));
  }

  viewer->update_drawable(
          "factors",
          std::make_shared<glk::ThinLines>(factor_lines, factor_colors),
          guik::VertexColor());
...
void InteractiveViewer::odometry_on_new_frame(
  ...
  invoke([this, new_frame] {
    ...
    const Eigen::Isometry3d pose =
            trajectory->odom2world(new_frame->T_world_sensor());
    viewer->update_drawable("current", cloud_buffer,
                            guik::FlatOrange(pose).set_point_scale(2.0f));
  });
}
 ...
 */
void InteractiveViewer::globalmap_on_insert_submap(
        const SubMap::ConstPtr& submap) {
  std::shared_ptr<Eigen::Isometry3d> pose(
          new Eigen::Isometry3d(submap->T_world_origin));
  invoke([this, submap, pose] {
    trajectory->update_anchor(submap->frames[submap->frames.size() / 2]->stamp,
 ...
 */
void InteractiveViewer::globalmap_on_update_submaps(
        const std::vector<SubMap::Ptr>& updated_submaps) {
  std::vector<Eigen::Isometry3d> poses(updated_submaps.size());
  std::transform(updated_submaps.begin(), updated_submaps.end(), poses.begin(),
                 [](const SubMap::ConstPtr& submap) {
                   return submap->T_world_origin;
                 });
 ...
 */
void InteractiveViewer::globalmap_on_smoother_update(
  ...
  for (const auto& factor : new_factors) {
    if (std::dynamic_pointer_cast<gtsam::BetweenFactor<gtsam::Pose3>>(factor)) {
      inserted_factors.push_back(std::make_tuple(
              FactorType::BETWEEN, factor->keys()[0], factor->keys()[1]));
    }
    if (std::dynamic_pointer_cast<gtsam_points::IntegratedMatchingCostFactor>(
                factor)) {
      inserted_factors.push_back(std::make_tuple(
              FactorType::MATCHING_COST, factor->keys()[0], factor->keys()[1]));
    }
#ifdef BUILD_GTSAM_POINTS_GPU
    if (std::dynamic_pointer_cast<gtsam_points::IntegratedVGICPFactorGPU>(
                factor)) {
      inserted_factors.push_back(std::make_tuple(
              FactorType::MATCHING_COST, factor->keys()[0], factor->keys()[1]));
    }
    ...
    if (std::dynamic_pointer_cast<gtsam::ImuFactor>(factor)) {
      inserted_factors.push_back(std::make_tuple(
              FactorType::IMU, factor->keys()[0], factor->keys()[2]));
    }
    ...
    if (std::dynamic_pointer_cast<gtsam::GPSFactor>(factor)) {
      ...
      inserted_gps_factors.push_back(
              std::make_tuple(symbol.index(), gps_factor->measurementIn()));
    }
  }

  invoke([this, inserted_factors, inserted_gps_factors] {
    global_factors.insert(global_factors.end(), inserted_factors.begin(),
                          inserted_factors.end());
    // global_gps_factors.insert(global_gps_factors.end(),
    // inserted_gps_factors.begin(), inserted_gps_factors.end());

    for (const auto& factor : inserted_gps_factors) {
      auto key = std::get<0>(factor);
      auto point = std::get<1>(factor);
      global_gps_factors[key] = point;
    }
  });
}
 ...
 */
void InteractiveViewer::globalmap_on_smoother_update_result(
        gtsam_points::ISAM2Ext& isam2,
...
bool InteractiveViewer::ok() const { return !request_to_terminate; }
