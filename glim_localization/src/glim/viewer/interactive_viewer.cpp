...
void InteractiveViewer::update_viewer() {
  ...
  for (const auto& factor: global_gps_factors) {
      ...
      viewer->update_drawable("gps_coord_" + std::to_string(factor.first),
            glk::Primitives::wire_sphere(),
            guik::FlatColor(0.5, 1.0f, 1.0, 0.8f, gps_pose * Eigen::UniformScaling<float>(0.25)));
  }
...
void InteractiveViewer::globalmap_on_smoother_update(gtsam_points::ISAM2Ext& isam2, gtsam::NonlinearFactorGraph& new_factors, gtsam::Values& new_values) {
  ...
  invoke([this, inserted_factors, inserted_gps_factors] { 
    ...
    // global_gps_factors.insert(global_gps_factors.end(), inserted_gps_factors.begin(), inserted_gps_factors.end()); 
