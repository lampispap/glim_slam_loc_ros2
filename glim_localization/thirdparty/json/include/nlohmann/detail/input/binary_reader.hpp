...
class binary_reader {
      ...
      return sax->parse_error(
              ...
              parse_error::create(
                      ...
                      exception_message(input_format_t::bson,
                                        concat("byte array length cannot be "
                                               "negative, is ",
                                               std::to_string(len)),
                                        "binary"),
                      nullptr));
        ...
        return sax->parse_error(
                ...
                parse_error::create(
                        ...
                        exception_message(
                                input_format_t::cbor,
                                concat("expected length specification "
                                       "(0x40-0x5B) or indefinite binary array "
                                       "type (0x5F); last byte: 0x",
                                       last_token),
                                "binary"),
                        nullptr));
      }
  ...
  */
  bool get_ubjson_string(string_t& result, const bool get_char = true) {
    ...
    if (input_format != input_format_t::bjdata) {
      message =
              "expected length type specification (U, i, I, l, L); last byte: "
              "0x" +
              last_token;
    } else {
      message =
          ...
          return sax->parse_error(
                  ...
                  parse_error::create(
                          ...
                          exception_message(input_format,
                                            "count in an optimized container "
                                            "must be positive",
                                            "size"),
                          nullptr));
        }
    ...
    if (input_format == input_format_t::bjdata && size_and_type.first != npos &&
      ...
      size_and_type.second &=
              ~(static_cast<char_int_type>(1)
                << 8);  // use bit 8 to indicate ndarray, here we remove the bit
                        // to restore the type marker
    ...
    if (input_format == input_format_t::bjdata && size_and_type.first != npos &&
      ...
      return sax->parse_error(
              ...
              parse_error::create(
                      112, chars_read,
                      exception_message(input_format,
                                        "BJData object does not support "
                                        "ND-array size in optimized format",
                                        "object"),
                      nullptr));
    }
