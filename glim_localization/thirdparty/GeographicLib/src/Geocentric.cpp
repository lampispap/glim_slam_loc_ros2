...
void Geocentric::IntForward(real lat,
                            real lon,
                            real h,
                            real& X,
                            real& Y,
...
void Geocentric::IntReverse(real X,
                            real Y,
                            real Z,
                            real& lat,
                            real& lon,
                            real& h,
                            real M[dim2_]) const {
  real R = Math::hypot(X, Y), slam = R != 0 ? Y / R : 0,
       clam = R != 0 ? X / R : 1;
  ...
  if (h > _maxrad) {
    ...
    real p = Math::sq(R / _a), q = _e2m * Math::sq(Z / _a),
         r = (p + q - _e4a) / 6;
    if (_f < 0) swap(p, q);
    if (!(_e4a * q == 0 && r <= 0)) {
      real
      ...
      real v = sqrt(Math::sq(u) +
           ...
           // subtraction.  Division by 0 not possible because uv > 0, w >= 0.
              k = uv / (sqrt(uv + Math::sq(w)) + w), k1 = _f >= 0 ? k : k - _e2,
           k2 = _f >= 0 ? k + _e2 : k, d = k1 * R / k2,
      ...
      real zz = sqrt((_f >= 0 ? _e4a - p : p) / _e2m),
           xx = sqrt(_f < 0 ? _e4a - p : p), H = Math::hypot(zz, xx);
      sphi = zz / H;
      cphi = xx / H;
...
void Geocentric::Rotation(
        real sphi, real cphi, real slam, real clam, real M[dim2_]) {
