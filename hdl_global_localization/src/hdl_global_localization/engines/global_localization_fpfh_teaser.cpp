...
GlobalLocalizationEngineFPFH_Teaser::GlobalLocalizationEngineFPFH_Teaser(
    ...
    : GlobalLocalizationEngineFPFH_RANSAC(private_nh) {}

GlobalLocalizationEngineFPFH_Teaser::~GlobalLocalizationEngineFPFH_Teaser() {}

void GlobalLocalizationEngineFPFH_Teaser::set_global_map(
        pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud) {
  this->global_map = cloud;
  this->global_map_features = extract_fpfh(cloud);

  evaluater.reset(new MatchingCostEvaluaterFlann());
  evaluater->set_target(
          this->global_map,
          private_nh.param<double>("ransac/max_correspondence_distance", 1.0));
}

GlobalLocalizationResults GlobalLocalizationEngineFPFH_Teaser::query(
        pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud,
  ...
  for (int i = 0; i < global_map->size(); i++) {
    target_cloud.push_back(
            {global_map->at(i).x, global_map->at(i).y, global_map->at(i).z});
  ...
  auto correspondences = matcher.calculateCorrespondences(
          source_cloud, target_cloud, source_features, target_features, false,
  ...
  params.rotation_max_iterations =
          private_nh.param<int>("teaser/rotation_max_iterations", 100);
  params.rotation_gnc_factor =
          private_nh.param<double>("teaser/rotation_gnc_factor", 1.4);
  ...
  params.rotation_cost_threshold =
          private_nh.param<double>("teaser/rotation_cost_threshold", 0.005);
  ...
  double error = evaluater->calc_matching_error(*cloud, transformation.matrix(),
                                                &inlier_fraction);

  std::vector<GlobalLocalizationResult::Ptr> results(1);
  results[0].reset(
          new GlobalLocalizationResult(error, inlier_fraction, transformation));
