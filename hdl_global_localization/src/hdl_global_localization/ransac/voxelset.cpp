...
VoxelSet::VoxelSet(double max_correspondence_distance)
    : resolution(max_correspondence_distance) {}
...
void VoxelSet::set_cloud(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud) {
  ...
  for (const auto& pt : *cloud) {
    ...
    for (const auto& offset : offsets) {
      ...
      if ((center - pt.getVector4fMap()).squaredNorm() <
...
double VoxelSet::matching_error(const pcl::PointCloud<pcl::PointXYZ>& cloud,
  ...
  *inlier_fraction = static_cast<double>(num_inliers) / cloud.size();
  return num_inliers ? errors / num_inliers
...
Eigen::Vector4f VoxelSet::voxel_center(const Eigen::Vector3i& coord) const {
  Eigen::Vector3f origin =
          (coord.template cast<float>().array() + 0.5) * resolution;
