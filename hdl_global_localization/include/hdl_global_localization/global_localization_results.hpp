...
struct GlobalLocalizationResult {
  ...
  GlobalLocalizationResult(double error,
...
struct GlobalLocalizationResults {
  GlobalLocalizationResults(
          const std::vector<GlobalLocalizationResult::Ptr>& results)
      : results(results) {}
  ...
  GlobalLocalizationResults& sort(int max_num_candidates) {
    ...
    // std::sort(results.begin(), results.end(), [](const auto& lhs, const auto&
    // rhs) { return lhs->error < rhs->error; });
