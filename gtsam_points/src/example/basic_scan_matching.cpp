 ...
 * @brief This example demonstrates how to perform simple frame-to-frame ICP
 * scan matching with gtsam_points.
 */
...
int main(int argc, char** argv) {
  ...
  const auto target_points =
          gtsam_points::read_points("data/kitti_00/000000.bin");
  const auto source_points =
          gtsam_points::read_points("data/kitti_00/000001.bin");

  // Create gtsam_points::PointCloudCPU instances that hold point data
  const auto target_frame =
          std::make_shared<gtsam_points::PointCloudCPU>(target_points);
  const auto source_frame =
          std::make_shared<gtsam_points::PointCloudCPU>(source_points);
  ...
  viewer->update_drawable(
          "target", std::make_shared<glk::PointCloudBuffer>(target_points),
          guik::FlatRed());
  viewer->update_drawable(
          "source", std::make_shared<glk::PointCloudBuffer>(source_points),
          guik::FlatGreen());
  viewer->update_drawable(
          "aligned", std::make_shared<glk::PointCloudBuffer>(source_points),
