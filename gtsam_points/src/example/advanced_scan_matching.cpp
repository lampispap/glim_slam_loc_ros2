 ...
 * @brief This example demonstrates how to use iVox structure to efficiently do
 * frame-to-map scan matching.
 */
...
int main(int argc, char** argv) {
  if (argc < 2) {
    std::cerr << "usage: advanced_scan_matching /path/to/your/kitti/00/velodyne"
              << std::endl;
    return 0;
  }
  ...
  for (int i = 0;; i++) {
    // Read points and replace the last element (w) with 1 for homogeneous
    // transformation
    const std::string points_path =
            (boost::format("%s/%06d.bin") % seq_path % i).str();
    auto points = gtsam_points::read_points4(points_path);
    if (points.empty()) {
      break;
    }
    std::for_each(points.begin(), points.end(),
                  [](Eigen::Vector4f& p) { p.w() = 1.0f; });
    ...
    if (i != 0) {
      ...
      values.insert(
              1,
              T_world_lidar);  // Source pose initial guess = Last sensor pose
      ...
      auto icp_factor = gtsam::make_shared<gtsam_points::IntegratedGICPFactor_<
              gtsam_points::iVox, gtsam_points::PointCloud>>(0, 1, ivox, frame,
                                                             ivox);
    ...
    viewer->update_drawable(
            guik::anon(), glk::Primitives::coordinate_system(),
            guik::VertexColor(T_world_lidar.matrix().cast<float>()));
    viewer->update_drawable(
            "current", std::make_shared<glk::PointCloudBuffer>(points),
            guik::FlatOrange(T_world_lidar.matrix().cast<float>())
                    .add("point_scale", 2.0f));
    viewer->update_drawable(
            "ivox",
            std::make_shared<glk::PointCloudBuffer>(ivox->voxel_points()),
            guik::Rainbow());
    if (!viewer->spin_once()) {
      break;
    }
