...
ContinuousTrajectory::ContinuousTrajectory(char symbol,
                                           double start_time,
                                           double end_time,
...
int ContinuousTrajectory::knot_max_id() const { return knot_id(end_time) + 2; }
...
gtsam::Pose3_ ContinuousTrajectory::pose(double t, const gtsam::Double_& t_) {
  ...
  const gtsam::Double_ p_ =
          (1.0 / knot_interval) * (t_ - gtsam::Double_(knot_t));
...
gtsam::Vector6_ ContinuousTrajectory::imu(double t,
  ...
  return gtsam_points::bspline_imu(gtsam::Symbol(symbol, knot_i), p_,
                                   knot_interval, g);
}

gtsam::Vector6 ContinuousTrajectory::imu(const gtsam::Values& values,
                                         double t,
                                         const Eigen::Vector3d& g) {
...
gtsam::Values ContinuousTrajectory::fit_knots(
  ...
  for (int i = 0; i <= knot_max_id(); i++) {
    ...
    while (pose_cursor < stamps.size() - 2 &&
           std::abs(stamps[pose_cursor] - t) >
                   std::abs(stamps[pose_cursor + 1] - t)) {
      pose_cursor++;
    }
  ...
  if (smoothness > 0.0) {
    const auto noise_model =
            gtsam::noiseModel::Isotropic::Precision(6, smoothness);
    for (int i = 0; i < knot_max_id(); i++) {
      graph.emplace_shared<gtsam::BetweenFactor<gtsam::Pose3>>(
              gtsam::Symbol(symbol, i), gtsam::Symbol(symbol, i + 1),
              gtsam::Pose3(), noise_model);
    }
  }
  ...
  for (int i = 0; i < stamps.size(); i++) {
    ...
    graph.emplace_shared<gtsam::ExpressionFactor<gtsam::Pose3>>(
            noise_model, gtsam::Pose3(poses[i].matrix()), pose_);
  }
