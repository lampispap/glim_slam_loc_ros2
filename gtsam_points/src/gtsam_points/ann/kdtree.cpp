...
struct traits<KdTree> {
  ...
  static const Eigen::Vector4d& point(const KdTree& tree, size_t i) {
    return tree.points[i];
  }
};

}  // namespace frame

KdTree::KdTree(const Eigen::Vector4d* points,
               int num_points,
...
size_t KdTree::knn_search(const double* pt,
                          size_t k,
                          size_t* k_indices,
                          double* k_sq_dists,
  ...
  return index->knn_search(Eigen::Map<const Eigen::Vector3d>(pt), k, k_indices,
                           k_sq_dists, setting);
}
