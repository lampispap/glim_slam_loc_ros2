...
PointCloudCPU::PointCloudCPU(const Eigen::Matrix<T, D, 1>* points,
                             int num_points) {
  add_points(points, num_points);
}

template PointCloudCPU::PointCloudCPU(const Eigen::Matrix<float, 3, 1>* points,
                                      int num_points);
template PointCloudCPU::PointCloudCPU(const Eigen::Matrix<float, 4, 1>* points,
                                      int num_points);
template PointCloudCPU::PointCloudCPU(const Eigen::Matrix<double, 3, 1>* points,
                                      int num_points);
template PointCloudCPU::PointCloudCPU(const Eigen::Matrix<double, 4, 1>* points,
                                      int num_points);
...
PointCloudCPU::Ptr PointCloudCPU::clone(const PointCloud& points) {
  ...
  for (const auto& attrib : points.aux_attributes) {
    ...
    const unsigned char* data_ptr =
            static_cast<const unsigned char*>(attrib.second.second);

    auto storage = std::make_shared<std::vector<unsigned char>>(points.size() *
                                                                elem_size);
    memcpy(storage->data(), data_ptr, elem_size * points.size());

    new_points->aux_attributes_storage[name] = storage;
    new_points->aux_attributes[name] =
            std::make_pair(elem_size, storage->data());
  }
...
// add_points
template <typename T, int D>
void PointCloudCPU::add_points(const Eigen::Matrix<T, D, 1>* points,
                               int num_points) {
  points_storage.resize(num_points, Eigen::Vector4d(0.0, 0.0, 0.0, 1.0));
  if (points) {
    for (int i = 0; i < num_points; i++) {
      points_storage[i].head<D>() =
              points[i].template head<D>().template cast<double>();
    }
  }
...
template void PointCloudCPU::add_points(
        const Eigen::Matrix<float, 3, 1>* points, int num_points);
template void PointCloudCPU::add_points(
        const Eigen::Matrix<float, 4, 1>* points, int num_points);
template void PointCloudCPU::add_points(
        const Eigen::Matrix<double, 3, 1>* points, int num_points);
template void PointCloudCPU::add_points(
        const Eigen::Matrix<double, 4, 1>* points, int num_points);

// add_normals
template <typename T, int D>
void PointCloudCPU::add_normals(const Eigen::Matrix<T, D, 1>* normals,
                                int num_points) {
  assert(num_points == size());
  normals_storage.resize(num_points, Eigen::Vector4d::Zero());
  if (normals) {
    for (int i = 0; i < num_points; i++) {
      normals_storage[i].head<D>() =
              normals[i].template head<D>().template cast<double>();
    }
  }
  this->normals = normals_storage.data();
}

template void PointCloudCPU::add_normals(
        const Eigen::Matrix<float, 3, 1>* normals, int num_points);
template void PointCloudCPU::add_normals(
        const Eigen::Matrix<float, 4, 1>* normals, int num_points);
template void PointCloudCPU::add_normals(
        const Eigen::Matrix<double, 3, 1>* normals, int num_points);
template void PointCloudCPU::add_normals(
        const Eigen::Matrix<double, 4, 1>* normals, int num_points);

// add_covs
template <typename T, int D>
void PointCloudCPU::add_covs(const Eigen::Matrix<T, D, D>* covs,
                             int num_points) {
  assert(num_points == size());
  covs_storage.resize(num_points, Eigen::Matrix4d::Zero());
  if (covs) {
    for (int i = 0; i < num_points; i++) {
      covs_storage[i].block<D, D>(0, 0) =
              covs[i].template block<D, D>(0, 0).template cast<double>();
    }
  }
  this->covs = covs_storage.data();
}

template void PointCloudCPU::add_covs(const Eigen::Matrix<float, 3, 3>* covs,
                                      int num_points);
template void PointCloudCPU::add_covs(const Eigen::Matrix<float, 4, 4>* covs,
                                      int num_points);
template void PointCloudCPU::add_covs(const Eigen::Matrix<double, 3, 3>* covs,
                                      int num_points);
template void PointCloudCPU::add_covs(const Eigen::Matrix<double, 4, 4>* covs,
                                      int num_points);
...
void PointCloudCPU::add_intensities(const T* intensities, int num_points) {
  ...
  if (intensities) {
    std::copy(intensities, intensities + num_points,
              intensities_storage.begin());
  }
  this->intensities = this->intensities_storage.data();
}

template void PointCloudCPU::add_intensities(const float* intensities,
                                             int num_points);
template void PointCloudCPU::add_intensities(const double* intensities,
                                             int num_points);
...
PointCloudCPU::Ptr PointCloudCPU::load(const std::string& path) {
  ...
  if (boost::filesystem::exists(path + "/points.bin")) {
    ...
    ifs.read(reinterpret_cast<char*>(frame->points),
             sizeof(Eigen::Vector4d) * frame->size());
    ...
    if (boost::filesystem::exists(path + "/times.bin")) {
      ...
      ifs.read(reinterpret_cast<char*>(frame->times),
    ...
    if (boost::filesystem::exists(path + "/normals.bin")) {
      ...
      ifs.read(reinterpret_cast<char*>(frame->normals),
               sizeof(Eigen::Vector4d) * frame->size());
    }
    ...
    if (boost::filesystem::exists(path + "/covs.bin")) {
      ...
      ifs.read(reinterpret_cast<char*>(frame->covs),
               sizeof(Eigen::Matrix4d) * frame->size());
    }
    ...
    if (boost::filesystem::exists(path + "/intensities.bin")) {
      ...
      ifs.read(reinterpret_cast<char*>(frame->intensities),
               sizeof(double) * frame->size());
    }
  } else if (boost::filesystem::exists(path + "/points_compact.bin")) {
    std::ifstream ifs(path + "/points_compact.bin",
    ...
    ifs.seekg(0, std::ios::beg);
    ifs.read(reinterpret_cast<char*>(points_f.data()),
             sizeof(Eigen::Vector3f) * frame->size());
    std::transform(points_f.begin(), points_f.end(), frame->points,
    ...
    if (boost::filesystem::exists(path + "/times_compact.bin")) {
      ...
      ifs.read(reinterpret_cast<char*>(times_f.data()),
    ...
    if (boost::filesystem::exists(path + "/normals_compact.bin")) {
      ...
      ifs.read(reinterpret_cast<char*>(normals_f.data()),
               sizeof(Eigen::Vector3f) * frame->size());
      std::transform(normals_f.begin(), normals_f.end(), frame->normals,
                     [](const Eigen::Vector3f& p) {
    ...
    if (boost::filesystem::exists(path + "/covs_compact.bin")) {
      ...
      ifs.read(reinterpret_cast<char*>(covs_f.data()),
               sizeof(Eigen::Matrix<float, 6, 1>) * frame->size());
      std::transform(covs_f.begin(), covs_f.end(), frame->covs,
                     [](const Eigen::Matrix<float, 6, 1>& c) {
    ...
    if (boost::filesystem::exists(path + "/intensities_compact.bin")) {
      ...
      ifs.read(reinterpret_cast<char*>(intensities_f.data()),
               sizeof(Eigen::Vector4f) * frame->size());
      std::copy(intensities_f.begin(), intensities_f.end(), frame->intensities);
    }

  } else {
    std::cerr << "error: " << path << " does not constain points(_compact)?.bin"
              << std::endl;
    return nullptr;
  }
  ...
  for (; itr != end; itr++) {
    ...
    if (elem_size * frame->size() != bytes) {
      std::cerr << "warning: elem_size=" << elem_size
                << " num_points=" << frame->size() << " bytes=" << bytes
                << std::endl;
...
// sample
PointCloudCPU::Ptr sample(const PointCloud::ConstPtr& frame,
  ...
  std::transform(indices.begin(), indices.end(), sampled->points,
                 [&](const int i) { return frame->points[i]; });

  if (frame->times) {
    sampled->times_storage.resize(indices.size());
    sampled->times = sampled->times_storage.data();
    std::transform(indices.begin(), indices.end(), sampled->times,
                   [&](const int i) { return frame->times[i]; });
  }

  if (frame->normals) {
    sampled->normals_storage.resize(indices.size());
    sampled->normals = sampled->normals_storage.data();
    std::transform(indices.begin(), indices.end(), sampled->normals,
                   [&](const int i) { return frame->normals[i]; });
  }

  if (frame->covs) {
    sampled->covs_storage.resize(indices.size());
    sampled->covs = sampled->covs_storage.data();
    std::transform(indices.begin(), indices.end(), sampled->covs,
                   [&](const int i) { return frame->covs[i]; });
  }

  if (frame->intensities) {
    sampled->intensities_storage.resize(indices.size());
    sampled->intensities = sampled->intensities_storage.data();
    std::transform(indices.begin(), indices.end(), sampled->intensities,
                   [&](const int i) { return frame->intensities[i]; });
  }
  ...
  for (const auto& attrib : frame->aux_attributes) {
    ...
    auto storage = std::make_shared<std::vector<unsigned char>>(indices.size() *
                                                                elem_size);
...
// random_sampling
PointCloudCPU::Ptr random_sampling(const PointCloud::ConstPtr& frame,
  ...
  std::sample(boost::counting_iterator<int>(0),
              boost::counting_iterator<int>(frame->size()),
              sample_indices.begin(), num_samples, mt);
...
// voxelgrid_sampling
PointCloudCPU::Ptr voxelgrid_sampling(const PointCloud::ConstPtr& frame,
  ...
  constexpr std::int64_t invalid_coord =
          std::numeric_limits<std::int64_t>::max();
  constexpr int coord_bit_size = 21;  // Bits to represent each voxel coordinate
                                      // (pack 21x3=63bits in 64bit int)
  constexpr size_t coord_bit_mask = (1 << 21) - 1;  // Bit mask
  constexpr int coord_offset =
          1 << (coord_bit_size -
                1);  // Coordinate offset to make values positive
...
#pragma omp parallel for num_threads(num_threads) schedule(guided, 32)
  for (std::int64_t i = 0; i < frame->size(); i++) {
    ...
    const Eigen::Array4i coord =
            fast_floor(frame->points[i] * inv_resolution) + coord_offset;
    ...
    const std::uint64_t bits =  //
            (static_cast<std::uint64_t>(coord[0] & coord_bit_mask)
             << (coord_bit_size * 0)) |  //
            (static_cast<std::uint64_t>(coord[1] & coord_bit_mask)
             << (coord_bit_size * 1)) |  //
            (static_cast<std::uint64_t>(coord[2] & coord_bit_mask)
             << (coord_bit_size * 2));
    coord_pt[i] = {bits, i};
  }
...
#pragma omp parallel for num_threads(num_threads) schedule(guided, 4)
  for (std::int64_t block_begin = 0; block_begin < coord_pt.size();
       block_begin += block_size) {
    std::vector<std::pair<std::uint64_t, size_t>*> sub_blocks;
    sub_blocks.reserve(block_size);

    const size_t block_end =
            std::min<size_t>(coord_pt.size(), block_begin + block_size);
    ...
    const size_t point_index_begin =
            num_points.fetch_add(sub_blocks.size() - 1);
    for (int i = 0; i < sub_blocks.size() - 1; i++) {
      ...
      if (frame->times) {
        ...
        downsampled->times_storage[point_index_begin + i] =
      ...
      if (frame->normals) {
        ...
        downsampled->normals_storage[point_index_begin + i] =
                normal_average.average();
      }
      ...
      if (frame->covs) {
        ...
        downsampled->covs_storage[point_index_begin + i] =
                cov_average.average();
      }
      ...
      if (frame->intensities) {
        ...
        downsampled->intensities_storage[point_index_begin + i] =
  ...
  if (!frame->aux_attributes.empty()) {
    std::cout << "warning: voxelgrid_sampling does not support aux attributes!!"
              << std::endl;
  }
...
// randomgrid_sampling
PointCloudCPU::Ptr randomgrid_sampling(const PointCloud::ConstPtr& frame,
                                       const double voxel_resolution,
                                       const double sampling_rate,
                                       std::mt19937& mt,
                                       int num_threads) {
  ...
  constexpr std::uint64_t invalid_coord =
...
#pragma omp parallel for num_threads(num_threads) schedule(guided, 128) \
  ...
  const size_t points_per_voxel =
          std::ceil((sampling_rate * frame->size()) / num_voxels);
...
#pragma omp parallel for num_threads(num_threads) schedule(guided, 4)
  for (std::int64_t block_begin = 0; block_begin < coord_pt.size();
    ...
    const auto flush_block_indices = [&] {
      if (block_indices.size() < points_per_voxel) {
        sub_indices.insert(sub_indices.end(), block_indices.begin(),
                           block_indices.end());
      } else {
        std::sample(block_indices.begin(), block_indices.end(),
                    std::back_inserter(sub_indices), points_per_voxel,
                    mts[omp_get_thread_num()]);
      }
      block_indices.clear();
    };
    ...
    std::copy(sub_indices.begin(), sub_indices.end(),
              indices.begin() + num_points.fetch_add(sub_indices.size()));
  }
  ...
  if (indices.size() > max_num_points) {
    ...
    std::sample(indices.begin(), indices.end(), sub_indices.begin(),
                max_num_points, mt);
...
PointCloudCPU::Ptr sort_by_time(const PointCloud::ConstPtr& frame) {
  ...
  return sort(frame, [&](const int lhs, const int rhs) {
...
PointCloudCPU::Ptr transform(
        ...
        const Eigen::Transform<double, 3, Eigen::Isometry>& transformation) {
  ...
  if (frame->covs) {
    for (int i = 0; i < frame->size(); i++) {
      transformed->covs[i] = transformation.matrix() * frame->covs[i] *
...
PointCloudCPU::Ptr transform(
        const PointCloud::ConstPtr& frame,
        const Eigen::Transform<float, 3, Eigen::Isometry>& transformation) {
  return transform<double, Eigen::Isometry>(frame,
...
PointCloudCPU::Ptr transform(
  ...
  if (frame->normals) {
    ...
    normal_matrix.block<3, 3>(0, 0) =
            transformation.linear().inverse().transpose();
    for (int i = 0; i < frame->size(); i++) {
...
void transform_inplace(
  ...
  if (frame.covs) {
    for (int i = 0; i < frame.size(); i++) {
      frame.covs[i] = transformation.matrix() * frame.covs[i] *
                      transformation.matrix().transpose();
    }
  }
}
...
void transform_inplace(
        PointCloud& frame,
        const Eigen::Transform<float, 3, Eigen::Isometry>& transformation) {
  transform_inplace<double, Eigen::Isometry>(frame,
                                             transformation.cast<double>());
}
...
void transform_inplace(
        PointCloud& frame,
        const Eigen::Transform<float, 3, Eigen::Affine>& transformation) {
  return transform_inplace<double, Eigen::Affine>(
          frame, transformation.cast<double>());
}

// statistical outlier removal
std::vector<int> find_inlier_points(const PointCloud::ConstPtr& frame,
                                    const std::vector<int>& neighbors,
                                    const int k,
                                    const double std_thresh) {
...
PointCloudCPU::Ptr remove_outliers(const PointCloud::ConstPtr& frame,
                                   const std::vector<int>& neighbors,
                                   const int k,
                                   const double std_thresh) {
...
#pragma omp parallel for schedule(guided, 8) num_threads(num_threads)
  for (int i = 0; i < frame->size(); i++) {
    ...
    kdtree.knn_search(frame->points[i].data(), k, k_neighbors.data(),
                      k_sq_dists.data());

    std::copy(k_neighbors.begin(), k_neighbors.end(),
              neighbors.begin() + i * k);
  }
