...
PointCloud::Ptr merge_frames(const std::vector<Eigen::Isometry3d>& poses,
  ...
  const int coord_offset =
          1 << (coord_bits - 1);  // Coordinate offset to make values positive
  ...
  const size_t num_all_points = std::accumulate(
          frames.begin(), frames.end(), 0,
          [](size_t sum, const auto& points) { return points->size() + sum; });
  ...
  for (size_t frame_id = 0; frame_id < frames.size(); frame_id++) {
    ...
    for (size_t point_id = 0; point_id < frame->size(); point_id++) {
      ...
      const Eigen::Array4i coord =
              fast_floor(point * inv_resolution) + coord_offset;
      ...
      const std::uint64_t index =
              (frame_id << frame_id_bits) | (point_id & frame_id_bitmask);
      coords_indices.emplace_back(
              (static_cast<std::uint64_t>(coord.x() & coord_bitmask)
               << (coord_bits * 0)) |
                      (static_cast<std::uint64_t>(coord.y() & coord_bitmask)
                       << (coord_bits * 1)) |
                      (static_cast<std::uint64_t>(coord.z() & coord_bitmask)
                       << (coord_bits * 2)),
              index);
    }
  }

  std::sort(coords_indices.begin(), coords_indices.end(),
  ...
  for (int i = 0; i < frames.size(); i++) {
    ...
    for (int j = 0; j < frames[i]->size(); j++) {
      ...
      merged->covs[dest] +=
              pose.matrix() * frames[i]->covs[j] * pose.matrix().transpose();
    }
  }
...
double overlap(const std::vector<GaussianVoxelMap::ConstPtr>& targets_,
               const PointCloud::ConstPtr& source,
               const std::vector<Eigen::Isometry3d>& Ts_target_source) {
  std::vector<GaussianVoxelMapCPU::ConstPtr> targets(targets_.size());
  for (int i = 0; i < targets_.size(); i++) {
    targets[i] =
            std::dynamic_pointer_cast<const GaussianVoxelMapCPU>(targets_[i]);
    if (!targets[i]) {
      std::cerr << "error: target CPU voxelmap has not been created!!"
                << std::endl;
    }
...
double overlap_auto(const GaussianVoxelMap::ConstPtr& target,
                    const PointCloud::ConstPtr& source,
                    const Eigen::Isometry3d& delta) {
#ifdef BUILD_GTSAM_POINTS_GPU
  if (source->points_gpu &&
      std::dynamic_pointer_cast<const GaussianVoxelMapGPU>(target)) {
...
double overlap_auto(const std::vector<GaussianVoxelMap::ConstPtr>& targets,
                    const PointCloud::ConstPtr& source,
                    const std::vector<Eigen::Isometry3d>& deltas) {
#ifdef BUILD_GTSAM_POINTS_GPU
  if (source->points_gpu && !targets.empty() &&
      std::dynamic_pointer_cast<const GaussianVoxelMapGPU>(targets[0])) {
