...
std::shared_ptr<gtsam::GaussianFactor> IntegratedMatchingCostFactor::linearize(
  ...
  if (is_binary) {
    factor.reset(new gtsam::HessianFactor(keys()[0], keys()[1], H_target,
                                          H_target_source, -b_target, H_source,
                                          -b_source, error));
  } else {
    factor.reset(
            new gtsam::HessianFactor(keys()[0], H_source, -b_source, error));
  }

  return factor;
}

Eigen::Isometry3d IntegratedMatchingCostFactor::calc_delta(
        const gtsam::Values& values) const {
  if (is_binary) {
