...
ICPFactorExpr::ICPFactorExpr(gtsam::Key target_key,
    ...
    : gtsam::NoiseModelFactor(noise_model,
      ...
      delta(gtsam::between(gtsam::Pose3_(target_key),
                           gtsam::Pose3_(source_key))),
      source(source),
      target_index(-1),
...
gtsam::Vector ICPFactorExpr::unwhitenedError(
...
gtsam::Point3_ ICPFactorExpr::calc_error() const {
  ...
  return gtsam::Point3_(target->points[target_index].head<3>()) -
         gtsam::transformFrom(delta, gtsam::Point3_(source));
}
...
gtsam::NonlinearFactorGraph::shared_ptr create_icp_factors(
  ...
  for (int i = 0; i < source->size(); i++) {
    gtsam::NonlinearFactor::shared_ptr factor(
            new ICPFactorExpr(target_key, source_key, target, target_tree,
                              source->points[i].head<3>(), noise_model));
    factors->add(factor);
  }
...
//
{
  auto factors = create_icp_factors(target_key, source_key, target, source,
                                    noise_model);
  return gtsam::NonlinearFactor::shared_ptr(
          new IntegratedICPFactorExpr(factors));
}
