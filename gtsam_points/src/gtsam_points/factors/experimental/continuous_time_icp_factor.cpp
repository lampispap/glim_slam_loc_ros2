...
CTICPFactorExpr::CTICPFactorExpr(
    ...
    : gtsam::NoiseModelFactor(noise_model,
...
gtsam::Vector CTICPFactorExpr::unwhitenedError(
        const gtsam::Values& values, std::vector<gtsam::Matrix>* H) const {
...
gtsam::Double_ CTICPFactorExpr::calc_error() const {
  ...
  gtsam::Point3_ error =
          gtsam::Point3_(target_pt.head<3>()) - transed_source_pt;
...
std::shared_ptr<gtsam::GaussianFactor> IntegratedCTICPFactorExpr::linearize(
  ...
  return gtsam::GaussianFactor::shared_ptr(new gtsam::HessianFactor(
          keys()[0], keys()[1], H.block<6, 6>(0, 0), H.block<6, 6>(0, 6),
          b.head<6>(), H.block<6, 6>(6, 6), b.tail<6>(), 0.0));
}

std::vector<Eigen::Vector3d> IntegratedCTICPFactorExpr::deskewed_source_points(
        const gtsam::Values& values) const {
...
gtsam::NonlinearFactorGraph::shared_ptr create_cticp_factors(
        ...
        const gtsam::SharedNoiseModel& noise_model) {
  gtsam::NonlinearFactorGraph::shared_ptr factors(
          new gtsam::NonlinearFactorGraph);
  std::shared_ptr<KdTree> target_tree(
          new KdTree(target->points, target->size()));
  ...
  for (int i = 0; i < source->size(); i++) {
    ...
    gtsam::NonlinearFactor::shared_ptr factor(new CTICPFactorExpr(
            source_t0_key, source_t1_key, target, target_tree, source_t0,
            source_t1, source_ti, source->points[i].head<3>(), noise_model));
...
IntegratedCTICPFactorExpr::shared_ptr create_integrated_cticp_factor(
        ...
        const gtsam::SharedNoiseModel& noise_model) {
  //
  auto factors = create_cticp_factors(source_t0_key, source_t1_key, target,
                                      source, noise_model);
  return IntegratedCTICPFactorExpr::shared_ptr(
          new IntegratedCTICPFactorExpr(factors));
}
