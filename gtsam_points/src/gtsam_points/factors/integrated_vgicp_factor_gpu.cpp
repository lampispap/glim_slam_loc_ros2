...
IntegratedVGICPFactorGPU::IntegratedVGICPFactorGPU(
    ...
    : gtsam_points::NonlinearFactorGPU(
              gtsam::KeyVector{target_key, source_key}),
      is_binary(true),
      fixed_target_pose(Eigen::Isometry3f::Identity()),
      target(std::dynamic_pointer_cast<const gtsam_points::GaussianVoxelMapGPU>(
              target)),
      source(source),
      derivatives(new IntegratedVGICPDerivatives(
              this->target, source, stream, temp_buffer)),
      linearized(false),
  ...
  if (!source->covs_gpu) {
    std::cerr << "error: GPU source covs have not been allocated!!"
              << std::endl;
    abort();
  }

  if (!this->target) {
    std::cerr << "error: GPU target voxels have not been created!!"
              << std::endl;
    abort();
  }
}
...
void IntegratedVGICPFactorGPU::set_inlier_update_thresh(double trans,
                                                        double angle) {
...
gtsam::NonlinearFactor::shared_ptr IntegratedVGICPFactorGPU::clone() const {
  if (is_binary) {
    return gtsam::make_shared<IntegratedVGICPFactorGPU>(
            keys()[0], keys()[1], target, source, nullptr, nullptr);
  }

  return gtsam::make_shared<IntegratedVGICPFactorGPU>(
          gtsam::Pose3(fixed_target_pose.cast<double>().matrix()), keys()[0],
          target, source, nullptr, nullptr);
}
...
Eigen::Isometry3f IntegratedVGICPFactorGPU::calc_delta(
        const gtsam::Values& values) const {
  if (!is_binary) {
    gtsam::Pose3 source_pose = values.at<gtsam::Pose3>(keys_[0]);
    gtsam::Pose3 delta =
            gtsam::Pose3(fixed_target_pose.inverse().cast<double>().matrix()) *
            source_pose;
...
double IntegratedVGICPFactorGPU::error(const gtsam::Values& values) const {
  ...
  if (evaluation_result) {
    ...
    std::cerr << "warning: computing error in sync mode seriously affects the "
                 "processing speed!!"
              << std::endl;
...
std::shared_ptr<gtsam::GaussianFactor> IntegratedVGICPFactorGPU::linearize(
        const gtsam::Values& values) const {
  linearized = true;
  ...
  if (linearization_result) {
    ...
    std::cerr << "warning: performing linearization in sync mode seriously "
  ...
  if (is_binary) {
    factor.reset(new gtsam::HessianFactor(
            keys_[0], keys_[1], l.H_target.cast<double>(),
            l.H_target_source.cast<double>(), -l.b_target.cast<double>(),
            l.H_source.cast<double>(), -l.b_source.cast<double>(), l.error));
  } else {
    factor.reset(new gtsam::HessianFactor(keys_[0], l.H_source.cast<double>(),
                                          -l.b_source.cast<double>(), l.error));
  }

  return factor;
}

void IntegratedVGICPFactorGPU::set_linearization_point(
        const gtsam::Values& values, void* lin_input_cpu) {
  // If -march=native is used and some GPU factor requests a storage size that
  // causes memory misalignment, the following lines that directly operates on
  // the input buffer may cause segfaults
  Eigen::Isometry3f* linearization_point =
          reinterpret_cast<Eigen::Isometry3f*>(lin_input_cpu);
  *linearization_point = calc_delta(values);
}

void IntegratedVGICPFactorGPU::set_evaluation_point(const gtsam::Values& values,
                                                    void* eval_input_cpu) {
  Eigen::Isometry3f* evaluation_point =
          reinterpret_cast<Eigen::Isometry3f*>(eval_input_cpu);
  *evaluation_point = calc_delta(values);
}

void IntegratedVGICPFactorGPU::issue_linearize(const void* lin_input_cpu,
                                               const void* lin_input_gpu,
                                               void* lin_output_gpu) {
  auto linearization_point =
          reinterpret_cast<const Eigen::Isometry3f*>(lin_input_cpu);
  auto linearization_point_gpu =
          reinterpret_cast<const Eigen::Isometry3f*>(lin_input_gpu);
...
void IntegratedVGICPFactorGPU::issue_compute_error(const void* lin_input_cpu,
  ...
  auto evaluation_point =
          reinterpret_cast<const Eigen::Isometry3f*>(eval_input_cpu);

  auto linearization_point_gpu =
          reinterpret_cast<const Eigen::Isometry3f*>(lin_input_gpu);
  auto evaluation_point_gpu =
          reinterpret_cast<const Eigen::Isometry3f*>(eval_input_gpu);

  auto error_gpu = reinterpret_cast<float*>(eval_output_gpu);

  derivatives->issue_compute_error(linearization_point_gpu,
                                   evaluation_point_gpu, error_gpu);
}

void IntegratedVGICPFactorGPU::store_computed_error(
        const void* eval_output_cpu) {
  auto evaluated = reinterpret_cast<const float*>(eval_output_cpu);
  evaluation_result = *evaluated;
}

void IntegratedVGICPFactorGPU::sync() { derivatives->sync_stream(); }
}  // namespace gtsam_points