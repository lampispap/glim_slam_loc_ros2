...
IncrementalFixedLagSmootherExtWithFallback::
        IncrementalFixedLagSmootherExtWithFallback(
                double smootherLag, const ISAM2Params& parameters)
...
IncrementalFixedLagSmootherExtWithFallback::Result
IncrementalFixedLagSmootherExtWithFallback::update(
  ...
  try {
    result =
            smoother->update(newFactors, newTheta, timestamps, factorsToRemove);
...
gtsam::Values IncrementalFixedLagSmootherExtWithFallback::calculateEstimate()
        const {
  try {
    return smoother->calculateEstimate();
  } catch (std::exception& e) {
    std::cerr << "warning: an exception was caught in fixed-lag smoother "
                 "calculateEstimate!!"
              << std::endl;
    std::cerr << "       : " << e.what() << std::endl;
...
const gtsam::Value&
IncrementalFixedLagSmootherExtWithFallback::calculateEstimate(
        gtsam::Key key) const {
  try {
...
void IncrementalFixedLagSmootherExtWithFallback::fallback_smoother() const {
  ...
  for (const auto& value : values) {
    const auto found = stamps.find(value.key);
    if (found == stamps.end()) {
      std::cerr << "warning: corresponding stamp is not found for "
                << gtsam::Symbol(value.key) << std::endl;
      continue;
    }
  ...
  for (const auto& min_stamp : min_stamps) {
    std::cout << boost::format("- symbol=%c min_stamp=%.6f fixation=%.6f") %
                         min_stamp.first % min_stamp.second %
                         (min_stamp.second + fixation_duration)
              << std::endl;
  }

  // connectivity check
  std::unordered_map<gtsam::Key, std::vector<gtsam::NonlinearFactor*>>
          factormap;
  for (const auto& factor : factors) {
    for (const auto key : factor->keys()) {
  ...
  for (const auto& max_id : max_ids) {
    ...
    if (found != factormap.end()) {
      ...
      traverse_queue.insert(traverse_queue.end(), found->second.begin(),
                            found->second.end());
    }
  }
  ...
  for (const auto& value : values) {
    if (!traversed_keys.count(value.key)) {
      std::cerr << "unreached key found:" << gtsam::Symbol(value.key)
                << std::endl;
  ...
  for (const auto& value : values) {
    ...
    if (symbol.chr() == 'x') {
      new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(
              value.key, gtsam::Pose3(value.value.cast<gtsam::Pose3>()),
  ...
  std::cout << "factors:" << factors.size()
            << " fixed:" << new_factors.size() - factors.size()
            << " values:" << values.size() << " stamps:" << stamps.size()
            << std::endl;
      ...
      if (!values.exists(key)) {
        ...
        for (const auto key : factor->keys()) {
          std::cout << "- " << gtsam::Symbol(key)
                    << " exists:" << values.exists(key) << std::endl;
        }
      }
    }
  }
  ...
  for (const auto& stamp : stamps) {
    if (!values.exists(stamp.first)) {
      std::cout << "illegal stamp found!! " << gtsam::Symbol(stamp.first)
                << std::endl;
    }
  }

  smoother.reset(new IncrementalFixedLagSmootherExt(smoother->smootherLag(),
                                                    smoother->params()));
