...
DenseLinearSystemBuilder::DenseLinearSystemBuilder(
        const gtsam::GaussianFactorGraph& gfg, const gtsam::Ordering& ordering)
...
gtsam::VectorValues SparseLinearSystemBuilderBase::delta(
        const Eigen::VectorXd& x) {
  return gtsam::VectorValues(x, scatter);
}

template <int BLOCK_SIZE>
SparseLinearSystemBuilder<BLOCK_SIZE>::SparseLinearSystemBuilder(
        const gtsam::GaussianFactorGraph& gfg, const gtsam::Ordering& ordering)
  ...
  std::unordered_map<std::pair<int, int>, BlockMatrix, PairHasher<int>>
          A_blocks;
  ...
  for (const auto& gf : gfg) {
    ...
    std::transform(gf->keys().begin(), gf->keys().end(), slots.begin(),
                   [&](const gtsam::Key key) { return block_slots[key]; });
    ...
    for (int i = 0, index_i = 0; i < gf->keys().size();
         index_i += slots[i++].dim) {
    ...
    for (int i = 0, index_i = 0; i < gf->keys().size();
         index_i += slots[i++].dim) {
      for (int j = i, index_j = index_i; j < gf->keys().size();
           index_j += slots[j++].dim) {
        const int dim_i = slots[i].dim;
        ...
        if (found == A_blocks.end()) {
          A_blocks.emplace_hint(
                  found, std::make_pair(global_index_i, global_index_j), H_ij);
        } else {
          found->second += H_ij;
        }
      }
  ...
  for (const auto& [index, H_ij] : A_blocks) {
    ...
    for (int k = 0; k < sH_ij.outerSize(); k++) {
      for (Eigen::SparseMatrix<double>::InnerIterator it(sH_ij, k); it; ++it) {
        triplets.emplace_back(index_i + it.row(), index_j + it.col(),
                              it.value());
      }
    }
  }
