...
struct ColoredGICPTestBase : public testing::Test {
  ...
  void test_factor(const gtsam::NonlinearFactor::shared_ptr& factor,
    ...
    EXPECT_LE(error_angle, error_angle_tol)
            << "[FORWARD] Too large rotation error " << tag;
    EXPECT_LE(error_trans, error_trans_tol)
            << "[FORWARD] Too large translation error" << tag;
    ...
    EXPECT_LE(error_angle, error_angle_tol)
            << "[BACKWARD] Too large rotation error" << tag;
    EXPECT_LE(error_trans, error_trans_tol)
            << "[BACKWARD] Too large translation error" << tag;
  }
...
TEST_F(ColoredGICPTestBase, Check) {
  gtsam_points::PointCloudCPU::Ptr target(
          new gtsam_points::PointCloudCPU(target_points));
  target->add_intensities(target_intensities);
  auto target_gradients =
          gtsam_points::IntensityGradients::estimate(target, 10, 50, 1);

  EXPECT_NE(target->normals, nullptr);
  EXPECT_NE(target->covs, nullptr);

  gtsam_points::PointCloud::Ptr target_ = target;
  auto target_gradients2 =
          gtsam_points::IntensityGradients::estimate(target_, 50, 1);

  gtsam_points::PointCloudCPU::Ptr source(
          new gtsam_points::PointCloudCPU(source_points));
  source->add_intensities(source_intensities);
  source->add_covs(
          gtsam_points::estimate_covariances(source->points, source->size()));

  std::shared_ptr<gtsam_points::KdTree> target_tree(
          new gtsam_points::KdTree(target->points, target->size()));
  ...
  test_factor(gtsam::make_shared<gtsam_points::IntegratedColoredGICPFactor>(
                      0, 1, target, source, target_tree, target_gradients),
              "DEFAULT");
  test_factor(gtsam::make_shared<gtsam_points::IntegratedColoredGICPFactor>(
                      0, 1, target, source, target_intensity_tree,
                      target_gradients),
