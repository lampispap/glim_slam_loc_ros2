...
TEST(BATest, DerivativeTest) {
  ...
  for (int num_points = 6; num_points <= 32; num_points *= 2) {
    ...
    for (int i = 0; i < num_points; i++) {
      points.push_back(
              Eigen::Vector3d(ndist(mt) * 0.1, ndist(mt) * 0.4, ndist(mt)));
    }
    ...
    for (int i = 0; i < num_points; i++) {
      ...
      for (int j = 0; j < num_points; j++) {
        Ha0.block<3, 3>(i * 3, j * 3) =
                feature.Hij<0>(points[i], points[j], i == j);
        Ha1.block<3, 3>(i * 3, j * 3) =
                feature.Hij<1>(points[i], points[j], i == j);
        Ha2.block<3, 3>(i * 3, j * 3) =
                feature.Hij<2>(points[i], points[j], i == j);
      }
    }
    ...
    Eigen::VectorXd x0 =
            Eigen::Map<Eigen::VectorXd>(points[0].data(), 3 * num_points);
    Eigen::VectorXd Jn0 = gtsam_points::numerical_jacobian(
            [&](const Eigen::VectorXd& x) { return calc_eigenvalue(0, x); },
            x0);
    Eigen::MatrixXd Hn0 = gtsam_points::numerical_hessian(
            [&](const Eigen::VectorXd& x) { return calc_eigenvalue(0, x); },
            x0);
    Eigen::VectorXd Jn1 = gtsam_points::numerical_jacobian(
            [&](const Eigen::VectorXd& x) { return calc_eigenvalue(1, x); },
            x0);
    Eigen::MatrixXd Hn1 = gtsam_points::numerical_hessian(
            [&](const Eigen::VectorXd& x) { return calc_eigenvalue(1, x); },
            x0);
    Eigen::VectorXd Jn2 = gtsam_points::numerical_jacobian(
            [&](const Eigen::VectorXd& x) { return calc_eigenvalue(2, x); },
            x0);
    Eigen::MatrixXd Hn2 = gtsam_points::numerical_hessian(
            [&](const Eigen::VectorXd& x) { return calc_eigenvalue(2, x); },
            x0);
    ...
    EXPECT_LT(err_J0, 1e-5) << "Too large Jacobian error for lambda_0 "
                            << num_points << "[pts]";
    EXPECT_LT(err_H0, 1e-3)
            << "Too large Hessian error for lambda_0 " << num_points << "[pts]";
    EXPECT_LT(err_J1, 1e-5) << "Too large Jacobian error for lambda_1 "
                            << num_points << "[pts]";
    EXPECT_LT(err_H1, 1e-3)
            << "Too large Hessian error for lambda_1 " << num_points << "[pts]";
    EXPECT_LT(err_J2, 1e-5) << "Too large Jacobian error for lambda_2 "
                            << num_points << "[pts]";
    EXPECT_LT(err_H2, 2e-3)
            << "Too large Hessian error for lambda_2 " << num_points << "[pts]";
  }
}
...
struct BATestBase : public testing::Test {
  virtual void SetUp() {
    ...
    EXPECT_EQ(ifs.is_open(), true)
            << "Failed to open " << data_path << "/graph.txt";

    gtsam::Values pose_noises;
    pose_noises.insert(0, gtsam::Pose3::Identity());
    pose_noises.insert(1, gtsam::Pose3::Expmap((gtsam::Vector6() << 0.02, 0.02,
                                                0.02, 0.2, 0.2, 0.2)
                                                       .finished()));
    pose_noises.insert(2, gtsam::Pose3::Expmap((gtsam::Vector6() << -0.02, 0.02,
                                                0.02, -0.2, 0.2, 0.2)
                                                       .finished()));
    pose_noises.insert(3, gtsam::Pose3::Expmap((gtsam::Vector6() << 0.02, -0.02,
                                                0.02, 0.2, -0.2, 0.2)
                                                       .finished()));
    pose_noises.insert(4, gtsam::Pose3::Expmap((gtsam::Vector6() << 0.02, 0.02,
                                                -0.02, 0.2, 0.2, -0.2)
                                                       .finished()));
    ...
    for (int i = 0; i < 5; i++) {
      ...
      poses.insert(
              i, gtsam::Pose3(pose.matrix()) * pose_noises.at<gtsam::Pose3>(i));
      poses_gt.insert(i, gtsam::Pose3(pose.matrix()));

      // Load points
      const std::string edge_path =
              (boost::format("%s/edges_%06d.bin") % data_path % (i * 10)).str();
      const std::string plane_path =
              (boost::format("%s/planes_%06d.bin") % data_path % (i * 10))
                      .str();
      ...
      edge_frames.push_back(gtsam_points::PointCloud::Ptr(
              new gtsam_points::PointCloudCPU(edge_points)));
      plane_frames.push_back(gtsam_points::PointCloud::Ptr(
              new gtsam_points::PointCloudCPU(plane_points)));
    }
  }
...
TEST_P(BAFactorTest, AlignmentTest) {
  ...
  for (const auto& center : edge_centers) {
    gtsam_points::BundleAdjustmentFactorBase::shared_ptr factor(
            new gtsam_points::EdgeEVMFactor());
