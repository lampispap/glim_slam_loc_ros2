...
int main(int argc, char** argv) {
  ...
  gtsam_points::IncrementalFixedLagSmootherExt smoother(smoother_lag,
                                                        isam2_params);
    ...
    if (token == "factor") {
      ...
      if (num_keys == 1) {
        new_factors.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(
                keys[0], gtsam::Pose3(), noise_model);
      } else if (num_keys == 2) {
        new_factors.emplace_shared<gtsam::BetweenFactor<gtsam::Pose3>>(
                keys[0], keys[1], gtsam::Pose3(), noise_model);
      }
    } else if (token == "theta") {
      gtsam::Key key;
      sst >> key;
      ...
      if (!new_stamps.empty()) {
        ...
        std::cout << boost::format("current:%.6f  horizon:%.6f") % now %
                             (now - smoother_lag)
                  << std::endl;
      }
      ...
      for (const auto& factor : new_factors) {
        for (const auto key : factor->keys()) {
          std::cout << "(" << gtsam::Symbol(key)
                    << boost::format(", %.6f)") % all_stamps[key] << " ";
        }
        std::cout << std::endl;
      }
      ...
      for (const auto& stamp : new_stamps) {
        std::cout << gtsam::Symbol(stamp.first) << " "
                  << boost::format("%.6f") % stamp.second << std::endl;
      }
