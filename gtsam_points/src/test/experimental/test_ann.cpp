...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < frame1->size(); i++) {
    size_t k_index;
    double k_sq_dist;
    size_t num_found = search.knn_search(frame1->points[i].data(), 1, &k_index,
                                         &k_sq_dist);
    ...
    std::cout << "---" << std::endl;
    std::cout << i << ":" << k_index << " " << frame1->points[i].transpose()
              << ":" << frame0->points[k_index].transpose()
              << " d=" << k_sq_dist << std::endl;
  ...
  for (int i = 0; i < frame1->size(); i++) {
    size_t k_index;
    double k_sq_dist;
    size_t num_found = kdtree.knn_search(frame1->points[i].data(), 1, &k_index,
                                         &k_sq_dist);
  }
  ...
  std::cout << std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1)
                       ...
                       1e6
            << "[msec]" << std::endl;
  std::cout << std::chrono::duration_cast<std::chrono::nanoseconds>(t3 - t2)
                               .count() /
                       1e6
            << "[msec]" << std::endl;
