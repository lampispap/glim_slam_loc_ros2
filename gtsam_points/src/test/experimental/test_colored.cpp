...
int main(int argc, char** argv) {
  ...
  delta.linear() =
          Eigen::AngleAxisd(0.05, Eigen::Vector3d(0.1, 0.2, 0.3).normalized())
                  .toRotationMatrix();
  ...
  auto target_gradients =
          gtsam_points::IntensityGradients::estimate(target, 10, 50);
  auto source_gradients =
          gtsam_points::IntensityGradients::estimate(source, 10, 50);
  // std::shared_ptr<gtsam_points::KdTree> target_tree(new
  // gtsam_points::KdTree(target->points, target->size()));
  std::shared_ptr<gtsam_points::IntensityKdTree> target_tree(
          new gtsam_points::IntensityKdTree(target->points, target->intensities,
                                            target->size()));
  ...
  auto f = gtsam::make_shared<gtsam_points::IntegratedColoredGICPFactor>(
          0, 1, target, source, target_tree, target_gradients);
  f->set_photometric_term_weight(0.75);
  // auto f = gtsam::make_shared<gtsam_points::IntegratedGICPFactor>(0, 1,
  // target, source);
  ...
  lm_params.callback =
          [&](const gtsam_points::LevenbergMarquardtOptimizationStatus& status,
            ...
            target_drawable.first->add(
                    "model_matrix", target_pose.matrix().cast<float>().eval());

            auto source_drawable = viewer->find_drawable("source");
            source_drawable.first->add(
                    "model_matrix", source_pose.matrix().cast<float>().eval());
  ...
  auto target_gradients = gtsam_points::IntensityGradients::estimate(target, 10,
  50); auto source_gradients =
  gtsam_points::IntensityGradients::estimate(source, 10, 50);
  ...
  gtsam::NonlinearFactorGraph graph;
  graph.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(0,
  gtsam::Pose3::Identity(), gtsam::noiseModel::Isotropic::Precision(6, 1e6));

  auto f = gtsam::make_shared<gtsam_points::IntegratedColoredGICPFactor>(0, 1,
  target, source, target_tree, target_gradients); f->set_num_threads(12);
  graph.add(f);
  // graph.emplace_shared<gtsam_points::IntegratedGICPFactor>(0, 1, target,
  source, target_tree);

  auto target_buffer = std::make_shared<glk::PointCloudBuffer>(target->points,
  target->size()); target_buffer->add_intensity(glk::COLORMAP::TURBO,
  target->intensities_storage);
  // target_buffer->add_color(target_gradients->intensity_gradients);
  viewer->update_drawable("target", target_buffer, guik::VertexColor());

  auto source_buffer = std::make_shared<glk::PointCloudBuffer>(source->points,
  source->size()); source_buffer->add_intensity(glk::COLORMAP::TURBO,
  source->intensities_storage); viewer->update_drawable("source", source_buffer,
  guik::VertexColor());

  gtsam_points::LevenbergMarquardtExtParams lm_params;
  lm_params.callback = [&](const
  gtsam_points::LevenbergMarquardtOptimizationStatus& status, const
  gtsam::Values& values) { viewer->append_text(status.to_string());

    gtsam::Pose3 target_pose = values.at<gtsam::Pose3>(0);
    gtsam::Pose3 source_pose = values.at<gtsam::Pose3>(1);

    auto target_drawable = viewer->find_drawable("target");
    target_drawable.first->add("model_matrix",
  target_pose.matrix().cast<float>().eval());

    auto source_drawable = viewer->find_drawable("source");
    source_drawable.first->add("model_matrix",
  source_pose.matrix().cast<float>().eval());

    viewer->spin_once();
  };

  gtsam_points::LevenbergMarquardtOptimizerExt optimizer(graph, values,
  lm_params); values = optimizer.optimize();
