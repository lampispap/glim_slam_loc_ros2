...
gtsam_points::ScanLineInformation estimate_scan_lines() {
  ...
  std::transform(
          ...
          [](const auto& p) { return Eigen::Vector4d(p[0], p[1], p[2], 1.0); });

  return gtsam_points::estimate_scan_lines(points.data(), points.size(), 64,
                                           0.2 * M_PI / 180.0);
}

int main(int argc, char** argv) {
  const std::string seq_path =
          "/home/koide/datasets/kitti/dataset/sequences/00/velodyne";
  ...
  for (int i = 0; i < 4500; i++) {
    ...
    auto edges_planes = gtsam_points::extract_edge_plane_points(
            scan_lines, points.data(), points.size());
    ...
    if (i != 0) {
      ...
      auto factor = gtsam::make_shared<gtsam_points::IntegratedLOAMFactor_<
              gtsam_points::iVox, gtsam_points::PointCloud>>(
              0, 1, ivox_edges, ivox_planes, edges, planes, ivox_edges,
              ivox_planes);
    ...
    viewer->update_drawable(
            ...
            guik::Rainbow());
    viewer->update_drawable(
            "edge_points", std::make_shared<glk::PointCloudBuffer>(edge_points),
            guik::FlatRed());
