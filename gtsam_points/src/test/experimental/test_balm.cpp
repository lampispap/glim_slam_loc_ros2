...
Eigen::MatrixXd numerical_hessian(const Func& f, const Eigen::VectorXd& x,
double eps = 1e-6) { const int N = x.size(); Eigen::MatrixXd h(N, N);
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < 5; i++) {
    auto points_path =
            (boost::format("data/newer_01/planes_%06d.bin") % (i * 10)).str();
    auto points = gtsam_points::read_points(points_path);
    frames.push_back(gtsam_points::PointCloudCPU::Ptr(
            new gtsam_points::PointCloudCPU(points)));
  ...
  for (int i = 0; i < frames.size(); i++) {
    graph.add(gtsam::PriorFactor<gtsam::Pose3>(
            i, values.at<gtsam::Pose3>(i),
            gtsam::noiseModel::Isotropic::Precision(6, 0.1)));
  }
  ...
  viewer->register_ui_callback("callback", [&] {
    if (ImGui::IsMouseClicked(1)) {
      ...
      if (depth > 0.0f) {
        center = viewer->unproject(Eigen::Vector2i(mouse_pos.x, mouse_pos.y),
                                   depth);
        viewer->update_drawable("center", glk::Primitives::sphere(),
                                guik::FlatColor(1.0, 0.0, 0.0, 0.4,
    ...
    if (ImGui::Button("add factor")) {
      gtsam_points::PlaneEVMFactor::shared_ptr plane_factor(
    ...
    if (ImGui::Button("optimize")) {
      ...
      lm_params.callback =
              [&](const gtsam_points::LevenbergMarquardtOptimizationStatus&
                ...
                for (int i = 0; i < frames.size(); i++) {
                  auto drawable =
                          viewer->find_drawable("frame_" + std::to_string(i));
                  drawable.first->add("model_matrix", values.at<gtsam::Pose3>(i)
                                                              .matrix()
                                                              .cast<float>()
                                                              .eval());
                }
