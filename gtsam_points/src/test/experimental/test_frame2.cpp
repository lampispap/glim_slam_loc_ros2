...
int main(int argc, char** argv) {
  ...
  const std::string data_path =
          "/home/koide/workspace/gtsam_points/data/kitti_07_dump";
  ...
  std::vector<std::vector<gtsam_points::GaussianVoxelMapGPU::ConstPtr>>
          voxelmaps(5);
  ...
  for (int i = 0; i < 5; i++) {
    auto points = gtsam_points::read_points(
    ...
    for (int j = 1; j < 5; j++) {
      for (auto& voxelmap : voxelmaps[i]) {
        ...
        graph.emplace_shared<gtsam_points::IntegratedVGICPFactorGPU>(
                i, j, voxelmap, frames[j], stream, buffer);
      }
    }
  }
  ...
  auto visualize = [&](const gtsam::Values& values) {
    for (int i = 0; i < 5; i++) {
      viewer->update_drawable(
              ...
              std::make_shared<glk::PointCloudBuffer>(frames[i]->points,
                                                      frames[i]->size()),
