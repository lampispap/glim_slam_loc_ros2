...
class ContinuousTimeDemo {
  ...
  ContinuousTimeDemo() {
    ...
    for (int i = 0; i < 3; i++) {
      const std::string times_path =
              (boost::format("%s/times_%02d.bin") % data_path % i).str();
      const std::string raw_path =
              (boost::format("%s/raw_%02d.bin") % data_path % i).str();
      const std::string deskewed_path =
              (boost::format("%s/deskewed_%02d.bin") % data_path % i).str();
      ...
      if (times.empty()) {
        ...
        std::cerr << "error: num of points mismatch " << deskewed_path
                  << std::endl;
        abort();
      }
      ...
      // Source frames
      auto raw_frame =
              std::make_shared<gtsam_points::PointCloudCPU>(raw_points);
      raw_frame->add_times(times);
      raw_frame->add_covs(gtsam_points::estimate_covariances(
              raw_frame->points, raw_frame->size()));
      raw_frames.push_back(raw_frame);

      // Target frames
      auto deskewed_frame =
              std::make_shared<gtsam_points::PointCloudCPU>(deskewed_points);
      deskewed_frame->add_covs(gtsam_points::estimate_covariances(
              deskewed_frame->points, deskewed_frame->size()));
      deskewed_frame->add_normals(gtsam_points::estimate_normals(
              deskewed_frame->points, deskewed_frame->covs,
              deskewed_frame->size()));
    ...
    viewer->register_ui_callback("callback", [this] {
      ...
      if (ImGui::Button("add noise")) {
        pose_noise = gtsam::Pose3::Expmap(gtsam::Vector6::Random() *
                                          pose_noise_scale);
        setup(data_id);
      }
      ...
      if (ImGui::Combo("DataID", &data_id, data_id_labels.data(),
                       data_id_labels.size())) {
        setup(data_id);
      }

      // Configurations
      ImGui::Combo("factor type", &factor_type, factor_types.data(),
                   factor_types.size());
      ...
      ImGui::DragFloat("pose trans noise scale", &trans_noise_scale, 10.0f,
                       0.0f);
      ImGui::DragFloat("max corresponding distance",
                       &max_correspondence_distance, 0.1f, 0.0f);
      ...
      if (ImGui::Button("optimize")) {
        ...
        optimization_thread =
                std::thread([this] { run_optimization(data_id); });
      }
    });
  }
  ...
  void setup(int data_id) {
    auto viewer = guik::LightViewer::instance();
    viewer->update_drawable("target",
                            std::make_shared<glk::PointCloudBuffer>(
                                    deskewed_frames[data_id]->points,
                                    deskewed_frames[data_id]->size()),
                            guik::FlatColor(0.0, 1.0, 0.0, 1.0));

    auto cloud_buffer = std::make_shared<glk::PointCloudBuffer>(
            raw_frames[data_id]->points, raw_frames[data_id]->size());
    std::vector<double> times(
            raw_frames[data_id]->times,
            raw_frames[data_id]->times + raw_frames[data_id]->size());
    cloud_buffer->add_intensity(glk::COLORMAP::BLUE_RED, times,
                                1.0 / times.back());
    viewer->update_drawable(
            "source", cloud_buffer,
            guik::VertexColor(pose_noise.matrix().cast<float>()));
  }
  ...
  void run_optimization(int data_id) {
    ...
    if (enable_pose_constraint) {
      ...
      noise_scales << gtsam::Vector3::Ones() * rot_noise_scale,
              gtsam::Vector3::Ones() * trans_noise_scale;
      graph.add(gtsam::BetweenFactor(
              0, 1, gtsam::Pose3::Identity(),
              gtsam::noiseModel::Isotropic::Precisions(noise_scales)));
    }

    // Create continuous time ICP factor
    gtsam::NonlinearFactor::shared_ptr factor;
    if (factor_types[factor_type] == std::string("CT-ICP")) {
      auto f = gtsam::make_shared<gtsam_points::IntegratedCT_ICPFactor>(
              0, 1, deskewed_frames[data_id], raw_frames[data_id]);
      f->set_max_correspondence_distance(max_correspondence_distance);
      factor = f;
    } else if (factor_types[factor_type] == std::string("CT-GICP")) {
      auto f = gtsam::make_shared<gtsam_points::IntegratedCT_GICPFactor>(
              0, 1, deskewed_frames[data_id], raw_frames[data_id]);
      f->set_max_correspondence_distance(max_correspondence_distance);
      factor = f;
    } else if (factor_types[factor_type] == std::string("CT-ICP-EXPR")) {
      auto noise_model = gtsam::noiseModel::Isotropic::Precision(1, 1.0);
      auto robust_model = gtsam::noiseModel::Robust::Create(
              gtsam::noiseModel::mEstimator::Huber::Create(
                      max_correspondence_distance),
              noise_model);
      factor = gtsam_points::create_integrated_cticp_factor(
              0, 1, deskewed_frames[data_id], raw_frames[data_id],
              robust_model);
    } else {
      std::cerr << "error: unknown factor type " << factor_types[factor_type]
                << std::endl;
    }
    ...
    lm_params.callback = [&](const gtsam_points::
                                     LevenbergMarquardtOptimizationStatus&
                                             status,
      ...
      auto cticp_factor =
              boost::dynamic_pointer_cast<gtsam_points::IntegratedCT_ICPFactor>(
                      factor);
      if (cticp_factor) {
        points = cticp_factor->deskewed_source_points(values);
      } else {
        auto cticp_factor_expr = boost::dynamic_pointer_cast<
                gtsam_points::IntegratedCTICPFactorExpr>(factor);
        auto deskewed = cticp_factor_expr->deskewed_source_points(values);
        std::transform(deskewed.begin(), deskewed.end(),
      ...
      for (int i = 0; i < 4; i++) {
        gtsam::Pose3 pose = gtsam_points::interpolate_pose(
                values.at<gtsam::Pose3>(0), values.at<gtsam::Pose3>(1),
