...
int main(int argc, char** argv) {
  ...
  viewer->register_ui_callback("ui", [&] {
    ...
    if (ImGui::Button("generate points") || target == nullptr) {
      ...
      delta.translation() = Eigen::Vector3d(0.25 * udist(mt), 0.25 * udist(mt),
                                            0.5 * udist(mt));
      delta.linear() =
              Eigen::AngleAxisd(0.3 * udist(mt),
                                Eigen::Vector3d(udist(mt), udist(mt), udist(mt))
                                        .normalized())
                      .toRotationMatrix();
      ...
      for (double x = -4.0; x <= 4.0; x += 0.02) {
        for (double y = -4.0; y < 4.0; y += 0.02) {
          ...
          target_points.push_back(
                  pt + Eigen::Vector4d(ndist(mt), ndist(mt), ndist(mt), 0.0));
          ...
          target_colors.push_back(glk::colormapf(glk::COLORMAP::TURBO,
                                                 target_intensities.back())
                                          .cast<double>());
      ...
      for (double x = -2.0; x <= 2.0; x += 0.04) {
        for (double y = -2.0; y < 2.0; y += 0.04) {
          ...
          source_points.push_back(delta * pt + Eigen::Vector4d(ndist(mt),
                                                               ndist(mt),
                                                               ndist(mt), 0.0));
      ...
      target_tree = std::make_shared<gtsam_points::KdTree>(target->points,
                                                           target->size());
      target_gradients =
      ...
      viewer->update_points("target", target_points,
                            guik::VertexColor().make_transparent())
              ->add_intensity(glk::COLORMAP::TURBO, target_intensities,
                              1.0 / intensity_scale);
      viewer->update_coord("target_coord", guik::VertexColor());
      viewer->update_points("source", source_points,
                            guik::VertexColor().set_point_scale(3.0f))
              ->add_intensity(glk::COLORMAP::JET, source_intensities,
                              1.0 / intensity_scale);
      viewer->update_coord("source_coord", guik::VertexColor(delta));
    }

    const std::vector<const char*> methods = {"ColoredICP", "ColoredGICP",
                                              "ICP (w/o color)"};
    ...
    if (ImGui::Button("align")) {
      ...
      switch (method) {
        case 0:
          graph.emplace_shared<gtsam_points::IntegratedPointToPlaneICPFactor>(
                  0, 1, target, source, target_tree);
          graph.emplace_shared<gtsam_points::IntegratedColorConsistencyFactor>(
                  0, 1, target, source, target_tree, target_gradients);
          break;

        case 1:
          graph.emplace_shared<gtsam_points::IntegratedColoredGICPFactor>(
                  0, 1, target, source, target_tree, target_gradients);
          break;
      ...
      lm_params.status_msg_callback = [&](const std::string& msg) {
        viewer->append_text(msg);
      };
      lm_params.callback =
              [&](const gtsam_points::LevenbergMarquardtOptimizationStatus&
                          status,
                  const gtsam::Values& values) {
                const Eigen::Isometry3d transform(
                        values.at<gtsam::Pose3>(1).matrix());
                viewer->invoke([=] {
                  viewer->find_drawable("source").first->set_model_matrix(
                          transform);
                  viewer->find_drawable("source_coord")
                          .first->set_model_matrix(delta * transform);
                });
              };

      optimization_thread.join();
      optimization_thread = std::thread([=] {
        gtsam_points::LevenbergMarquardtOptimizerExt(graph, values, lm_params)
                .optimize();
      });
    }
  });
