...
class BundleAdjustmentDemo {
  ...
  BundleAdjustmentDemo() {
    ...
    viewer->enable_vsync();
    viewer->append_text(
            "Usage: Right click a map point and then click [add factor] and "
            "[optimize]");
    ...
    if (!ifs) {
      std::cerr << "error: failed to open " << data_path + "/graph.txt"
                << std::endl;
      abort();
    }
    ...
    for (int i = 0; i < 5; i++) {
      ...
      edge_frames.push_back(
              std::make_shared<gtsam_points::PointCloudCPU>(edge_points));
      plane_frames.push_back(
              std::make_shared<gtsam_points::PointCloudCPU>(plane_points));

      viewer->update_drawable(
              "edge_" + std::to_string(i),
              std::make_shared<glk::PointCloudBuffer>(edge_points),
              guik::Rainbow());
      viewer->update_drawable(
              "plane_" + std::to_string(i),
              std::make_shared<glk::PointCloudBuffer>(plane_points),
              guik::Rainbow());
    }
    ...
    viewer->register_ui_callback("callback", [this] {
      ...
      if (ImGui::IsMouseClicked(1)) {
        ...
        if (depth >= 0.0f && depth < 1.0f) {
          Eigen::Vector3f pt = viewer->unproject(
                  Eigen::Vector2i(mouse_pos.x, mouse_pos.y), depth);
          center = pt.cast<double>();
          viewer->update_drawable("sphere", glk::Primitives::sphere(),
                                  guik::FlatColor(1.0f, 0.0f, 0.0f, 0.4f,
                                                  Eigen::Translation3f(pt))
                                          .make_transparent());
        }
      }
      ...
      if (ImGui::Button("initialize graph")) {
        ...
        poses.clear();
        for (int i = 0; i < 5; i++) {
          gtsam::Pose3 noise = gtsam::Pose3::Expmap(gtsam::Vector6::Random() *
                                                    pose_noise_scale);
          ...
          graph.add(gtsam::PriorFactor<gtsam::Pose3>(
                  i, pose, gtsam::noiseModel::Isotropic::Precision(6, 0.1)));
        }
        update_viewer(poses);
      }
      ...
      if (factor_types[factor_type] == std::string("EVM")) {
        ...
        ImGui::Combo("feature", &edge_plane, edge_plane_labels.data(),
                     edge_plane_labels.size());
      } else {
        // Force set the selection mode to the plane mode, because LSQ BA only
        // supports plane features
        edge_plane = 1;
      }
  ...
  void update_viewer(const gtsam::Values& values) {
    guik::LightViewer::instance()->invoke([=] {
      ...
      for (int i = 0; i < 5; i++) {
        ...
        auto plane_drawable =
                viewer->find_drawable("plane_" + std::to_string(i));
        edge_drawable.first->add("model_matrix",
                                 pose.matrix().cast<float>().eval());
        plane_drawable.first->add("model_matrix",
                                  pose.matrix().cast<float>().eval());

        viewer->update_drawable("coord_" + std::to_string(i),
                                glk::Primitives::coordinate_system(),
                                guik::VertexColor(pose.matrix().cast<float>()));
      }
    });
  }
  ...
  void add_factor() {
    ...
    if (factor->num_points() > 6) {
      ...
      guik::LightViewer::instance()->append_text(
              feature_type + " factor is created with " +
              std::to_string(factor->num_points()) + " points");
      graph.add(factor);
    } else {
      guik::LightViewer::instance()->append_text(
              "ignoring the factor with too few points: " +
              std::to_string(factor->num_points()));
    }
  }
  ...
  void run_optimization() {
    ...
    gtsam_points::LevenbergMarquardtOptimizerExt optimizer(graph, poses,
                                                           lm_params);
    poses = optimizer.optimize();
  }

  private:
  std::vector<gtsam_points::PointCloud::Ptr>
          edge_frames;  // Frames containing edge points
  std::vector<gtsam_points::PointCloud::Ptr>
          plane_frames;  // Frames containing plane points
