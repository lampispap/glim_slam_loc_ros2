...
class GaussianVoxelMapGPU : public GaussianVoxelMap {
   ...
   * @param max_bucket_scan_count    Maximum number of open hashmap addressing
   * trials. If a target voxel coord is not found in successive N buckets, it is
   * considered to be not found. Setting a larger value makes the hashing slower
   * but results in a smaller memory footprint.
   * @param target_points_drop_rate  This parameter allows to drop a fraction of
   * points during the bucket creation.
   */
  GaussianVoxelMapGPU(float resolution,
  ...
  virtual double voxel_resolution() const override {
    return voxelmap_info.voxel_resolution;
  }
...
std::vector<Eigen::Vector3f> download_voxel_means(
        const GaussianVoxelMapGPU& voxelmap, CUstream_st* stream = nullptr);
std::vector<Eigen::Matrix3f> download_voxel_covs(
        const GaussianVoxelMapGPU& voxelmap, CUstream_st* stream = nullptr);
