...
/**
 * @brief Standard point cloud class that holds only pointers to point
 * attributes
 * @note  If you don't want to manage the lifetime of point data by yourself,
 * use gtsam_points::PointCloudCPU.
 */
struct PointCloud {
  public:
  ...
  bool has_times()
          const;  ///< Check if the point cloud has per-point timestamps
  ...
  bool has_intensities()
          const;  ///< Check if the point cloud has point intensities
  ...
  bool check_intensities()
          const;  ///< Warn if the point cloud doesn't have intensities

  bool has_times_gpu()
          const;  ///< Check if the point cloud has per-point timestamps on GPU
  bool has_points_gpu() const;  ///< Check if the point cloud has points on GPU
  bool has_normals_gpu()
          const;  ///< Check if the point cloud has point normals on GPU
  bool has_covs_gpu()
          const;  ///< Check if the point cloud has point covariances on GPU
  bool has_intensities_gpu()
          const;  ///< Check if the point cloud has point intensities on GPU

  bool check_times_gpu()
          const;  ///< Warn if the point cloud doesn't have times on GPU
  bool check_points_gpu()
          const;  ///< Warn if the point cloud doesn't have points on GPU
  bool check_normals_gpu()
          const;  ///< Warn if the point cloud doesn't have normals on GPU
  bool check_covs_gpu()
          const;  ///< Warn if the point cloud doesn't have covs on GPU
  bool check_intensities_gpu()
          const;  ///< Warn if the point cloud doesn't have intensities on GPU

  /**
   * @brief Get the pointer to an aux attribute
   * @param  attrib Attribute name
   * @return Returns the pointer to it if the specified attribute exists.
  ...
  const T* aux_attribute(const std::string& attrib) const {
    ...
    if (found == aux_attributes.end()) {
      std::cerr << "warning: attribute " << attrib << " not found!!"
                << std::endl;
      return nullptr;
    }

    if (sizeof(T) != found->second.first) {
      std::cerr << "warning: attribute element size mismatch!! attrib:"
                << attrib << " size:" << found->second.first
                << " requested:" << sizeof(T) << std::endl;
    }
  ...
  /**
   * @brief Save the point cloud data with a compact representation without
   * unnecessary fields (e.g., the last element of homogeneous coordinates).
   * @param path
   */
  ...
  double* times;  ///< Per-point timestamp w.r.t. the first point (should be
                  ///< sorted)
