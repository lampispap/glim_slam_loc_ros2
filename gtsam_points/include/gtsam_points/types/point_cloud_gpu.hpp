...
struct PointCloudGPU : public PointCloudCPU {
  ...
  PointCloudGPU(const std::vector<Eigen::Matrix<T, D, 1>, Alloc>& points)
      : PointCloudGPU(points.data(), points.size()) {}
  ...
  // Deep copy
  static PointCloudGPU::Ptr clone(const PointCloud& frame,
  ...
  template <typename T, int D>
  void add_points(const Eigen::Matrix<T, D, 1>* points,
                  int num_points,
                  CUstream_st* stream = 0) {
  ...
  template <typename T, int D>
  void add_points_gpu(const Eigen::Matrix<T, D, 1>* points,
                      int num_points,
                      CUstream_st* stream = 0);
  template <typename T, int D, typename Alloc>
  ...
  template <typename T, int D>
  void add_normals(const Eigen::Matrix<T, D, 1>* normals,
                   int num_points,
                   CUstream_st* stream = 0) {
  ...
  template <typename T, int D>
  void add_normals_gpu(const Eigen::Matrix<T, D, 1>* normals,
                       int num_points,
                       CUstream_st* stream = 0);
  template <typename T, int D, typename Alloc>
  void add_normals_gpu(
          const std::vector<Eigen::Matrix<T, D, 1>, Alloc>& normals,
          CUstream_st* stream = 0) {
    add_normals_gpu(normals.data(), normals.size(), stream);
  }

  template <typename T, int D>
  void add_covs(const Eigen::Matrix<T, D, D>* covs,
                int num_points,
                CUstream_st* stream = 0) {
  ...
  template <typename T, int D>
  void add_covs_gpu(const Eigen::Matrix<T, D, D>* covs,
                    int num_points,
                    CUstream_st* stream = 0);
  template <typename T, int D, typename Alloc>
  ...
  template <typename T>
  void add_intensities(const T* intensities,
                       int num_points,
                       CUstream_st* stream = 0) {
  ...
  template <typename T>
  void add_intensities_gpu(const T* intensities,
                           int num_points,
                           CUstream_st* stream = 0);
  template <typename T>
...
std::vector<Eigen::Vector3f> download_points_gpu(
        const gtsam_points::PointCloud& frame, CUstream_st* stream = nullptr);
std::vector<Eigen::Matrix3f> download_covs_gpu(
        const gtsam_points::PointCloud& frame, CUstream_st* stream = nullptr);
std::vector<Eigen::Vector3f> download_normals_gpu(
        const gtsam_points::PointCloud& frame, CUstream_st* stream = nullptr);
std::vector<float> download_intensities_gpu(
        const gtsam_points::PointCloud& frame, CUstream_st* stream = nullptr);
