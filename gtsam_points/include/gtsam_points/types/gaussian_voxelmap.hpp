 ...
 * @return                  Overlap rate
 */
double overlap(const GaussianVoxelMap::ConstPtr& target,
 ...
 * @return         Overlap rate
 */
double overlap(const std::vector<GaussianVoxelMap::ConstPtr>& targets,
               const PointCloud::ConstPtr& source,
               const std::vector<Eigen::Isometry3d>& Ts_target_source);

/// @brief Calculate the fraction of points fell in targets' voxels
template <typename VoxelMapPtr>
std::enable_if_t<!std::is_same_v<VoxelMapPtr, GaussianVoxelMap::ConstPtr>,
                 double>
overlap(const std::vector<VoxelMapPtr>& targets,
        const PointCloud::ConstPtr& source,
        const std::vector<Eigen::Isometry3d>& Ts_target_source) {
  const std::vector<GaussianVoxelMap::ConstPtr> targets_(targets.begin(),
                                                         targets.end());
 ...
 * @return         Overlap rate
 */
double overlap_gpu(const GaussianVoxelMap::ConstPtr& target,
                   const PointCloud::ConstPtr& source,
...
/// @brief Calculate the fraction of points fell in target voxels on GPU
double overlap_gpu(const PointCloud::ConstPtr& target,
                   const PointCloud::ConstPtr& source,
                   const Eigen::Isometry3d& delta,
                   CUstream_st* stream = 0);
 ...
 * @return         Overlap rate
 */
double overlap_gpu(const std::vector<GaussianVoxelMap::ConstPtr>& targets,
                   const PointCloud::ConstPtr& source,
...
std::enable_if_t<!std::is_same_v<VoxelMapPtr, GaussianVoxelMap::ConstPtr>,
                 double>
overlap_gpu(const std::vector<VoxelMapPtr>& targets,
            const PointCloud::ConstPtr& source,
            const std::vector<Eigen::Isometry3d>& deltas,
            CUstream_st* stream = 0) {
  const std::vector<GaussianVoxelMap::ConstPtr> targets_(targets.begin(),
...
std::enable_if_t<!std::is_same_v<VoxelMapPtr, GaussianVoxelMap::ConstPtr>,
                 double>
overlap_auto(const std::vector<VoxelMapPtr>& targets,
             const PointCloud::ConstPtr& source,
             const std::vector<Eigen::Isometry3d>& deltas) {
  const std::vector<GaussianVoxelMap::ConstPtr> targets_(targets.begin(),
                                                         targets.end());
