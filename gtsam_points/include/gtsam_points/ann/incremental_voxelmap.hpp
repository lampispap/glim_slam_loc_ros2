...
///        This class supports incremental point cloud insertion and LRU-based
///        voxel deletion.
/// @note  This class can be used as a point cloud as well as a neighbor search
/// structure.
/// @note  For the compatibility with other nearest neighbor search methods,
/// this implementation returns indices that encode the voxel and point IDs.
///        The first ```point_id_bits``` (e.g., 32) bits of a point index
///        represent the point ID, and the rest ```voxel_id_bits``` (e.g., 32)
///        bits represent the voxel ID that contains the point. The specified
///        point can be looked up by `voxelmap.point(index)`;
...
struct IncrementalVoxelMap : public NearestNeighborSearch {
  ...
  void set_lru_clear_cycle(const int lru_clear_cycle) {
    this->lru_clear_cycle = lru_clear_cycle;
  }
  /// @brief LRU cache horizon.
  void set_lru_horizon(const int lru_horizon) {
    this->lru_horizon = lru_horizon;
  }
  /// @brief Neighboring voxel search mode (1, 7, 19, or 27).
  void set_neighbor_voxel_mode(const int mode) {
    offsets = neighbor_offsets(mode);
  }
  /// @brief Voxel setting.
  typename VoxelContents::Setting& voxel_insertion_setting() {
    return voxel_setting;
  }
  ...
  /// @brief Calculate the global point index from the voxel index and the point
  /// index.
  inline size_t calc_index(const size_t voxel_id, const size_t point_id) const {
    return (voxel_id << point_id_bits) | point_id;
  }
  inline size_t voxel_id(const size_t i) const {
    return i >> point_id_bits;
  }  ///< Extract the point ID from a global index.
  inline size_t point_id(const size_t i) const {
    return i & ((1ull << point_id_bits) - 1);
  ...
  decltype(auto) normal(const size_t i) const {
    return frame::normal(flat_voxels[voxel_id(i)]->second, point_id(i));
  }
  decltype(auto) cov(const size_t i) const {
    return frame::cov(flat_voxels[voxel_id(i)]->second, point_id(i));
  }
  decltype(auto) intensity(const size_t i) const {
    return frame::intensity(flat_voxels[voxel_id(i)]->second, point_id(i));
  }
  ...
  std::vector<Eigen::Vector3i> neighbor_offsets(
          const int neighbor_voxel_mode) const;
  ...
  static constexpr int point_id_bits =
          32;  ///< Use the first 32 bits for point id
  static constexpr int voxel_id_bits =
          64 - point_id_bits;  ///< Use the remaining bits for voxel id
  double inv_leaf_size;        ///< Inverse of the voxel size
  std::vector<Eigen::Vector3i> offsets;  ///< Neighbor voxel offsets

  size_t lru_horizon;  ///< LRU horizon size. Voxels that have not been accessed
                       ///< for lru_horizon steps are deleted.
  size_t lru_clear_cycle;  ///< LRU clear cycle. Voxel deletion is performed
                           ///< every lru_clear_cycle steps.
  size_t lru_counter;      ///< LRU counter. Incremented every step.

  typename VoxelContents::Setting voxel_setting;  ///< Voxel setting.
  std::vector<std::shared_ptr<std::pair<VoxelInfo, VoxelContents>>>
          flat_voxels;  ///< Voxel contents.
  std::unordered_map<Eigen::Vector3i, size_t, XORVector3iHash>
          voxels;  ///< Voxel index map.
};
...
struct traits<IncrementalVoxelMap<VoxelContents>> {
  static bool has_points(const IncrementalVoxelMap<VoxelContents>& ivox) {
    return ivox.has_points();
  }
  static bool has_normals(const IncrementalVoxelMap<VoxelContents>& ivox) {
    return ivox.has_normals();
  }
  static bool has_covs(const IncrementalVoxelMap<VoxelContents>& ivox) {
    return ivox.has_covs();
  }
  static bool has_intensities(const IncrementalVoxelMap<VoxelContents>& ivox) {
    return ivox.has_intensities();
  }

  static const Eigen::Vector4d& point(
          const IncrementalVoxelMap<VoxelContents>& ivox, size_t i) {
    return ivox.point(i);
  }
  ...
  static const Eigen::Matrix4d& cov(
          const IncrementalVoxelMap<VoxelContents>& ivox, size_t i) {
    return ivox.cov(i);
  }
  static double intensity(const IncrementalVoxelMap<VoxelContents>& ivox,
                          size_t i) {
