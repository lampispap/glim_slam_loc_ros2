...
void IncrementalVoxelMap<VoxelContents>::insert(const PointCloud& points) {
  ...
  for (size_t i = 0; i < points.size(); i++) {
    const Eigen::Vector3i coord =
            fast_floor(points.points[i] * inv_leaf_size).template head<3>();

    auto found = voxels.find(coord);
    if (found == voxels.end()) {
      auto voxel = std::make_shared<std::pair<VoxelInfo, VoxelContents>>(
              VoxelInfo(coord, lru_counter), VoxelContents());
  ...
  if ((++lru_counter) % lru_clear_cycle == 0) {
    ...
    auto remove_counter = std::remove_if(
            ...
            [&](const std::shared_ptr<std::pair<VoxelInfo, VoxelContents>>&
                        voxel) {
...
template <typename VoxelContents>
size_t IncrementalVoxelMap<VoxelContents>::knn_search(
        const double* pt,
        size_t k,
        size_t* k_indices,
        double* k_sq_dists,
...
IncrementalVoxelMap<VoxelContents>::neighbor_offsets(
  ...
  switch (neighbor_voxel_mode) {
    ...
    default:
      std::cerr << "error: invalid neighbor voxel mode " << neighbor_voxel_mode
                << std::endl;
      std::cerr << "     : neighbor voxel mode must be 1, 7, 19, or 27"
                << std::endl;
      return std::vector<Eigen::Vector3i>();
  }
}
...
bool IncrementalVoxelMap<VoxelContents>::has_points() const {
  return flat_voxels.empty() ? false
                             : frame::has_points(flat_voxels.front()->second);
}

template <typename VoxelContents>
bool IncrementalVoxelMap<VoxelContents>::has_normals() const {
  return flat_voxels.empty() ? false
                             : frame::has_normals(flat_voxels.front()->second);
}

template <typename VoxelContents>
bool IncrementalVoxelMap<VoxelContents>::has_covs() const {
  return flat_voxels.empty() ? false
                             : frame::has_covs(flat_voxels.front()->second);
}

template <typename VoxelContents>
bool IncrementalVoxelMap<VoxelContents>::has_intensities() const {
  return flat_voxels.empty()
                 ? false
                 : frame::has_intensities(flat_voxels.front()->second);
}

template <typename VoxelContents>
std::vector<Eigen::Vector4d> IncrementalVoxelMap<VoxelContents>::voxel_points()
        const {
  std::vector<Eigen::Vector4d> points;
  points.reserve(flat_voxels.size() * 10);
  visit_points([&](const auto& voxel, const int i) {
    points.emplace_back(frame::point(voxel, i));
  });
  return points;
}

template <typename VoxelContents>
std::vector<Eigen::Vector4d> IncrementalVoxelMap<VoxelContents>::voxel_normals()
        const {
  std::vector<Eigen::Vector4d> normals;
  normals.reserve(flat_voxels.size() * 10);
  visit_points([&](const auto& voxel, const int i) {
    normals.emplace_back(frame::normal(voxel, i));
  });
  return normals;
}

template <typename VoxelContents>
std::vector<Eigen::Matrix4d> IncrementalVoxelMap<VoxelContents>::voxel_covs()
        const {
  std::vector<Eigen::Matrix4d> covs;
  covs.reserve(flat_voxels.size() * 10);
  visit_points([&](const auto& voxel, const int i) {
    covs.emplace_back(frame::cov(voxel, i));
  });
  return covs;
}

template <typename VoxelContents>
std::vector<double> IncrementalVoxelMap<VoxelContents>::voxel_intensities()
        const {
  std::vector<double> intensities;
  intensities.reserve(flat_voxels.size() * 10);
  visit_points([&](const auto& voxel, const int i) {
    intensities.emplace_back(frame::intensity(voxel, i));
  });
  return intensities;
}
...
PointCloudCPU::Ptr IncrementalVoxelMap<VoxelContents>::voxel_data() const {
  ...
  frame->normals = frame->normals_storage.empty()
                           ? nullptr
                           : frame->normals_storage.data();
  frame->covs =
          frame->covs_storage.empty() ? nullptr : frame->covs_storage.data();
  frame->intensities = frame->intensities_storage.empty()
                               ? nullptr
                               : frame->intensities_storage.data();
