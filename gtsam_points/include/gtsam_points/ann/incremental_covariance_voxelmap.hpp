...
struct IncrementalCovarianceVoxelMap
    : public IncrementalVoxelMap<IncrementalCovarianceContainer> {
  public:
  /// @brief Constructor.
  ...
  /// @brief Set the number of warmup cycles. Covariances of new points in this
  /// period are not re-evaluated every frame.
  void set_warmup_cycles(int warmup_cycles);
  /// @brief Set the number of lowrate update cycles. Covariances of invalid
  /// points are re-evaluated every this period.
  void set_lowrate_cycles(int lowrate_cycles);
  /// @brief Set the age threshold for removing invalid points. Invalid points
  ...
  /// @brief Find k-nearest neighbors. This only finds neighbors with valid
  /// covariances.
  ...
  /// @brief Find k-nearest neighbors. This finds neighbors regardless of the
  /// validity of covariances.
  size_t knn_search_force(
          const double* pt,
          size_t k,
          size_t* k_indices,
          double* k_sq_dists,
  ...
  /// @brief Get valid point indices. If num_threads is -1, the member variable
  /// num_threads is used.
  ...
  std::vector<Eigen::Vector4d> voxel_normals(
          const std::vector<size_t>& indices) const;
  /// @brief Get covariances from indices.
  std::vector<Eigen::Matrix4d> voxel_covs(
          const std::vector<size_t>& indices) const;
  ...
  int min_num_neighbors;  ///< Minimum number of neighbors for covariance
                          ///< estimation.
  ...
  int remove_invalid_age_thresh;   ///< Age threshold for removing invalid
                                   ///< points.
  ...
  std::deque<RunningStatistics<Eigen::Array3d>>
          eig_stats;  ///< Running statistics for eigenvalues.
};
...
struct traits<IncrementalCovarianceVoxelMap> {
  static bool has_points(const IncrementalCovarianceVoxelMap& ivox) {
    return ivox.has_points();
  }
  static bool has_normals(const IncrementalCovarianceVoxelMap& ivox) {
    return ivox.has_normals();
  }
  static bool has_covs(const IncrementalCovarianceVoxelMap& ivox) {
    return ivox.has_covs();
  }
  static bool has_intensities(const IncrementalCovarianceVoxelMap& ivox) {
    return ivox.has_intensities();
  }

  static const Eigen::Vector4d& point(const IncrementalCovarianceVoxelMap& ivox,
                                      size_t i) {
    return ivox.point(i);
  }
  static const Eigen::Vector4d& normal(
          const IncrementalCovarianceVoxelMap& ivox, size_t i) {
    return ivox.normal(i);
  }
  static const Eigen::Matrix4d& cov(const IncrementalCovarianceVoxelMap& ivox,
                                    size_t i) {
    return ivox.cov(i);
  }
  static double intensity(const IncrementalCovarianceVoxelMap& ivox, size_t i) {
