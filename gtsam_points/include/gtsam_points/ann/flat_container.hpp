...
struct traits<FlatContainer> {
  ...
  static bool has_points(const FlatContainer& frame) {
    return !frame.points.empty();
  }
  static bool has_normals(const FlatContainer& frame) {
    return !frame.normals.empty();
  }
  static bool has_covs(const FlatContainer& frame) {
    return !frame.covs.empty();
  }
  static bool has_intensities(const FlatContainer& frame) {
    return !frame.intensities.empty();
  }

  static const Eigen::Vector4d& point(const FlatContainer& frame, size_t i) {
    return frame.points[i];
  }
  static const Eigen::Vector4d& normal(const FlatContainer& frame, size_t i) {
    return frame.normals[i];
  }
  static const Eigen::Matrix4d& cov(const FlatContainer& frame, size_t i) {
    return frame.covs[i];
  }
  static double intensity(const FlatContainer& frame, size_t i) {
