 ...
 *        Zhang, "Iterative Point Matching for Registration of Free-Form Curve",
 * IJCV1994
 */
...
class IntegratedICPFactor_ : public gtsam_points::IntegratedMatchingCostFactor {
   ...
   * @param use_point_to_plane  If true, use point-to-plane distance instead of
  ...
  IntegratedICPFactor_(gtsam::Key target_key,
                       gtsam::Key source_key,
                       const std::shared_ptr<const TargetFrame>& target,
                       const std::shared_ptr<const SourceFrame>& source,
                       bool use_point_to_plane = false);
  ...
  /**
   * @brief Create a unary ICP factor between a fixed target pose and an active
   * source pose.
  ...
  /// Create a unary ICP factor between a fixed target pose and an active source
  /// pose.
  IntegratedICPFactor_(const gtsam::Pose3& fixed_target_pose,
                       gtsam::Key source_key,
                       const std::shared_ptr<const TargetFrame>& target,
                       const std::shared_ptr<const SourceFrame>& source,
                       bool use_point_to_plane = false);
...
template <typename TargetFrame = gtsam_points::PointCloud,
          typename SourceFrame = gtsam_points::PointCloud>
class IntegratedPointToPlaneICPFactor_
    : public gtsam_points::IntegratedICPFactor_<TargetFrame, SourceFrame> {
  public:
  using shared_ptr = boost::shared_ptr<
          IntegratedPointToPlaneICPFactor_<TargetFrame, SourceFrame>>;
  ...
  IntegratedPointToPlaneICPFactor_(
          ...
          const std::shared_ptr<const SourceFrame>& source,
          const std::shared_ptr<NearestNeighborSearch>& target_tree)
      : IntegratedICPFactor_<TargetFrame, SourceFrame>(
                target_key, source_key, target, source, target_tree, true) {}
  ...
  IntegratedPointToPlaneICPFactor_(
          ...
          const std::shared_ptr<const SourceFrame>& source)
      : IntegratedICPFactor_<TargetFrame, SourceFrame>(
                target_key, source_key, target, source, true) {}
};
