 ...
 *        The 4th element (intensity) will be simply ignored if a standard
 * gtsam_points::KdTree is given while it can provide additional distance
 * information between points if gtsam_points::IntensityKdTree is used
 *
 * @note  While the use of IntensityKdTree significantly improves the
 * convergence speed, it can affect optimization stability in some cases
 *
...
class IntegratedColorConsistencyFactor_
    : public gtsam_points::IntegratedMatchingCostFactor {
  public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  using shared_ptr = boost::shared_ptr<
          IntegratedColorConsistencyFactor_<TargetFrame, SourceFrame>>;

  /**
   * @brief Create a binary color consistency factor between target and source
   * poses.
  ...
  /**
   * @brief Create a unary color consistency factor between a fixed target pose
   * and an active source pose.
   * @param target_key          Target key
   * @param source_key          Source key
   * @param target              Target point cloud frame
   * @param source              Source point cloud frame
  ...
  /// @note If your GTSAM is built with TBB, linearization is already
  /// multi-threaded
  ...
  ///        Correspondences with distances larger than this will be rejected
  ///        (i.e., correspondence trimming).
  void set_max_correspondence_distance(double d) {
    max_correspondence_distance_sq = d * d;
  }

  /// @brief Set the weight for the photometric error term.
  void set_photometric_term_weight(double w) { photometric_term_weight = w; }

  /// @brief Correspondences are updated only when the displacement from the
  /// last update point is larger than these threshold values.
  /// @note  Default values are angle=trans=0 and correspondences are updated
  /// every linearization call.
  ...
  virtual void update_correspondences(
          const Eigen::Isometry3d& delta) const override;
