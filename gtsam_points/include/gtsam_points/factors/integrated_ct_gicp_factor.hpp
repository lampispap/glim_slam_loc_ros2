 ...
 * Closure", 2021 Segal et al., "Generalized-ICP", RSS2005
 */
...
class IntegratedCT_GICPFactor_
    : public IntegratedCT_ICPFactor_<TargetFrame, SourceFrame> {
  public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  using shared_ptr =
          boost::shared_ptr<IntegratedCT_GICPFactor_<TargetFrame, SourceFrame>>;
   ...
   * @param source          Source point cloud
   */
  IntegratedCT_GICPFactor_(gtsam::Key source_t0_key,
                           gtsam::Key source_t1_key,
                           const std::shared_ptr<const TargetFrame>& target,
                           const std::shared_ptr<const SourceFrame>& source);
