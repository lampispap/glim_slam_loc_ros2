...
class IntegratedGICPFactor_
  ...
  IntegratedGICPFactor_(gtsam::Key target_key,
                        gtsam::Key source_key,
                        const std::shared_ptr<const TargetFrame>& target,
                        const std::shared_ptr<const SourceFrame>& source);

  /**
   * @brief Create a unary GICP factor between a fixed target pose and an active
   * source pose.
  ...
  ///< Create a unary GICP factor between a fixed target pose and an active
  ///< source pose.
  IntegratedGICPFactor_(const gtsam::Pose3& fixed_target_pose,
                        gtsam::Key source_key,
                        const std::shared_ptr<const TargetFrame>& target,
                        const std::shared_ptr<const SourceFrame>& source);
  ...
  /// with a distance smaller than the trimming threshold.
  double inlier_fraction() const {
    const int outliers =
            std::count(correspondences.begin(), correspondences.end(), -1);
