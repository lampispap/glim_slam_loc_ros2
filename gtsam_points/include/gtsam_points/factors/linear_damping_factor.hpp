 ...
 *        It causes a damping effect (like lambda in LM optimization) and helps
...
class LinearDampingFactor : public gtsam::LinearContainerFactor {
  ...
  LinearDampingFactor(gtsam::Key key, int dim, double mu)
      : gtsam::LinearContainerFactor(
                gtsam::HessianFactor(key,
                                     mu * gtsam::Matrix::Identity(dim, dim),
                                     gtsam::Vector::Zero(dim),
                                     0.0)) {}
  ...
  void serialize(ARCHIVE& ar, const unsigned int /*version*/) {
    ...
    ar& boost::serialization::make_nvp(
            ...
            boost::serialization::base_object<gtsam::LinearContainerFactor>(
                    *this));
  }
};
