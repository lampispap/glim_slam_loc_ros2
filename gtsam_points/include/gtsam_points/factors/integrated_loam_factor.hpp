...
/**
 * @brief Scan matching factor based on the combination of point-to-plane and
 * point-to-edge distances
 *
 * Zhang and Singh, "Low-drift and real-time lidar odometry and mapping",
 * Autonomous Robots, 2017 Zhang and Singh, "LOAM: LiDAR Odometry and Mapping in
 * Real-time", RSS2014 Tixiao and Brendan, "LeGO-LOAM: Lightweight and
 * Ground-Optimized Lidar Odometry and Mapping on Variable Terrain", IROS2018
 */
template <typename TargetFrame = gtsam_points::PointCloud,
          typename SourceFrame = gtsam_points::PointCloud>
class IntegratedLOAMFactor_
    : public gtsam_points::IntegratedMatchingCostFactor {
  public:
  GTSAM_MAKE_ALIGNED_OPERATOR_NEW
  using shared_ptr =
          boost::shared_ptr<IntegratedLOAMFactor_<TargetFrame, SourceFrame>>;
  ...
  std::unique_ptr<IntegratedPointToEdgeFactor_<TargetFrame, SourceFrame>>
          edge_factor;
  std::unique_ptr<IntegratedPointToPlaneFactor_<TargetFrame, SourceFrame>>
          plane_factor;
};
...
using IntegratedPointToPlaneFactor =
        gtsam_points::IntegratedPointToPlaneFactor_<>;
using IntegratedPointToEdgeFactor =
        gtsam_points::IntegratedPointToEdgeFactor_<>;
