...
IntegratedICPFactor_<TargetFrame, SourceFrame>::IntegratedICPFactor_(
  ...
  if (!frame::has_points(*target) ||
      (use_point_to_plane && !frame::has_normals(*target))) {
    std::cerr << "error: target frame doesn't have required attributes for icp"
              << std::endl;
    abort();
  }

  if (!frame::has_points(*source)) {
    std::cerr << "error: source frame doesn't have required attributes for icp"
              << std::endl;
    abort();
  }
...
IntegratedICPFactor_<TargetFrame, SourceFrame>::IntegratedICPFactor_(
        ...
        const std::shared_ptr<const SourceFrame>& source,
        bool use_point_to_plane)
    : gtsam_points::IntegratedICPFactor_<TargetFrame, SourceFrame>(
              target_key,
              source_key,
              target,
              source,
...
template <typename TargetFrame, typename SourceFrame>
void IntegratedICPFactor_<TargetFrame, SourceFrame>::update_correspondences(
...
#pragma omp parallel for num_threads(num_threads) schedule(guided, 8)
  for (int i = 0; i < frame::size(*source); i++) {
    ...
    size_t k_index = -1;
    double k_sq_dist = -1;
    size_t num_found = target_tree->knn_search(
            pt.data(), 1, &k_index, &k_sq_dist, max_correspondence_distance_sq);
