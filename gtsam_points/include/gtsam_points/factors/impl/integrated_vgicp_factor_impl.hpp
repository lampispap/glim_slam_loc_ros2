...
IntegratedVGICPFactor_<SourceFrame>::IntegratedVGICPFactor_(
    ...
    : gtsam_points::IntegratedMatchingCostFactor(target_key, source_key),
      num_threads(1),
      target_voxels(std::dynamic_pointer_cast<const GaussianVoxelMapCPU>(
              target_voxels)),
      source(source) {
  //
...
template <typename SourceFrame>
void IntegratedVGICPFactor_<SourceFrame>::update_correspondences(
...
#pragma omp parallel for num_threads(num_threads) schedule(guided, 8)
  for (int i = 0; i < frame::size(*source); i++) {
    ...
    if (voxel_id < 0) {
      ...
      Eigen::Matrix4d RCR =
              (voxel->cov + delta.matrix() * frame::cov(*source, i) *
