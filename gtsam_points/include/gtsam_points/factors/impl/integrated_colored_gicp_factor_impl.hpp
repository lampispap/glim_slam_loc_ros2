...
template <typename TargetFrame,
          ...
          typename IntensityGradients>
IntegratedColoredGICPFactor_<TargetFrame, SourceFrame, IntensityGradients>::
        IntegratedColoredGICPFactor_(
                ...
                const std::shared_ptr<const IntensityGradients>&
                        target_gradients)
  ...
  //
  if (!frame::has_points(*target) || !frame::has_normals(*target) ||
      !frame::has_covs(*target) || !frame::has_intensities(*target)) {
...
template <typename TargetFrame,
          typename SourceFrame,
          typename IntensityGradients>
void IntegratedColoredGICPFactor_<TargetFrame,
                                  SourceFrame,
                                  IntensityGradients>::
...
double
IntegratedColoredGICPFactor_<TargetFrame, SourceFrame, IntensityGradients>::
        evaluate(const Eigen::Isometry3d& delta,
...
#pragma omp parallel for num_threads(num_threads)                      \
        reduction(+ : sum_errors_geom) reduction(+ : sum_errors_photo) \
        schedule(guided, 8)
  for (int i = 0; i < frame::size(*source); i++) {
    ...
    const auto& gradient_B =
            frame::intensity_gradient(*target_gradients, target_index);
    ...
    sum_errors_geom += 0.5 * error_geom.transpose() * geometric_term_weight *
                       mahalanobis[i] * error_geom;

    // photometric error
    const Eigen::Vector4d projected =
            transed_A - (transed_A - mean_B).dot(normal_B) * normal_B;
    const Eigen::Vector4d offset = projected - mean_B;
    const double error_photo =
            intensity_B + gradient_B.dot(offset) - intensity_A;

    sum_errors_photo +=
            0.5 * error_photo * photometric_term_weight * error_photo;
    ...
    Eigen::Matrix<double, 4, 6> J_transed_target =
    ...
    Eigen::Matrix<double, 4, 6> J_transed_source =
            Eigen::Matrix<double, 4, 6>::Zero();
    J_transed_source.block<3, 3>(0, 0) =
    ...
    Hs_target[thread_num] += J_egeom_target.transpose() *
                             geometric_term_weight * mahalanobis[i] *
                             J_egeom_target;
    Hs_source[thread_num] += J_egeom_source.transpose() *
                             geometric_term_weight * mahalanobis[i] *
                             J_egeom_source;
    Hs_target_source[thread_num] += J_egeom_target.transpose() *
                                    geometric_term_weight * mahalanobis[i] *
                                    J_egeom_source;
    bs_target[thread_num] += J_egeom_target.transpose() *
                             geometric_term_weight * mahalanobis[i] *
                             error_geom;
    bs_source[thread_num] += J_egeom_source.transpose() *
                             geometric_term_weight * mahalanobis[i] *
                             error_geom;

    // photometric error derivatives
    Eigen::Matrix<double, 4, 4> J_projected_transed =
            Eigen::Matrix4d::Identity() - normal_B * normal_B.transpose();
    ...
    Eigen::Matrix<double, 1, 4> J_ephoto_transed =
            J_ephoto_offset * J_offset_transed;

    Eigen::Matrix<double, 1, 6> J_ephoto_target =
            J_ephoto_transed * J_transed_target;
    Eigen::Matrix<double, 1, 6> J_ephoto_source =
            J_ephoto_transed * J_transed_source;

    Hs_target[thread_num] += J_ephoto_target.transpose() *
                             photometric_term_weight * J_ephoto_target;
    Hs_source[thread_num] += J_ephoto_source.transpose() *
                             photometric_term_weight * J_ephoto_source;
    Hs_target_source[thread_num] += J_ephoto_target.transpose() *
                                    photometric_term_weight * J_ephoto_source;
    bs_target[thread_num] +=
            J_ephoto_target.transpose() * photometric_term_weight * error_photo;
    bs_source[thread_num] +=
            J_ephoto_source.transpose() * photometric_term_weight * error_photo;
  }
