...
struct GTSAM_EXPORT DoglegOptimizerImplExt {
   ...
   * of an original nonlinear error function (a NonlinearFactorGraph) \f$ f(x)
   * \f$. The quadratic approximation is represented as a GaussianBayesNet \f$
   * bayesNet \f$, which is obtained by eliminating a GaussianFactorGraph
   * resulting from linearizing the nonlinear factor graph \f$ f(x) \f$.  Thus,
   * \f$ M(\delta x) \f$ is \f[ M(\delta x) = (R \delta x - d)^T (R \delta x -
   * d) \f] where \f$ R \f$ and \f$ d \f$ together are a Bayes' net or Bayes'
   * tree. \f$ R \f$ is upper-triangular and \f$ d \f$ is a vector, in GTSAM
   * represented as a BayesNet<GaussianConditional> (GaussianBayesNet) or
  ...
  static IterationResult Iterate(double delta,
   ...
   * resulting from linearizing the nonlinear factor graph \f$ f(x) \f$.  Thus,
   * \f$ M(\delta x) \f$ is \f[ M(\delta x) = (R \delta x - d)^T (R \delta x -
   * d) \f] where \f$ R \f$ and \f$ d \f$ together are a Bayes' net.  \f$ R \f$
   * is upper-triangular and \f$ d \f$ is a vector, in GTSAM represented as a
   ...
   */
  static gtsam::VectorValues ComputeDoglegPoint(double delta,
                                                const gtsam::VectorValues& dx_u,
                                                const gtsam::VectorValues& dx_n,
                                                const bool verbose = false);
   ...
   * \f$ \| (1-\tau)\delta x_u + \tau\delta x_n \| = \delta \f$, where \f$
   * \delta \f$ is the trust region radius.
   * @param delta Trust region radius
   * @param x_u Steepest descent minimizer
   * @param x_n Newton's method minimizer
   */
  static gtsam::VectorValues ComputeBlend(double delta,
                                          const gtsam::VectorValues& x_u,
                                          const gtsam::VectorValues& x_n,
                                          const bool verbose = false);
};

/* ************************************************************************* */
template <class M, class F, class VALUES>
typename DoglegOptimizerImplExt::IterationResult
DoglegOptimizerImplExt::Iterate(double delta,
                                TrustRegionAdaptationMode mode,
                                const gtsam::VectorValues& dx_u,
                                const gtsam::VectorValues& dx_n,
                                const M& Rd,
  ...
  enum {
    ...
    INCREASED_DELTA,
    DECREASED_DELTA
  } lastAction = NONE;  // Used to prevent alternating between increasing and
                        // decreasing in one iteration
  while (stay) {
    gttic(Dog_leg_point);
    ...
    if (verbose)
      std::cout << "delta = " << delta << ", dx_d_norm = " << result.dx_d.norm()
                << std::endl;
    ...
    if (verbose)
      std::cout << std::setprecision(15) << "f error: " << f_error << " -> "
                << result.f_error << std::endl;
    if (verbose)
      std::cout << std::setprecision(15) << "M error: " << M_error << " -> "
    ...
    const double rho =
            std::abs(f_error - result.f_error) < 1e-15 ||
                            std::abs(M_error - new_M_error) < 1e-15
                    ? 0.5
                    : (f_error - result.f_error) / (M_error - new_M_error);

    if (verbose)
      std::cout << std::setprecision(15) << "rho = " << rho << std::endl;

    if (rho >= 0.75) {
      // M agrees very well with f, so try to increase lambda
      const double dx_d_norm = result.dx_d.norm();
      const double newDelta =
              std::max(delta, 3.0 * dx_d_norm);  // Compute new delta
      ...
      if (mode == ONE_STEP_PER_ITERATION || mode == SEARCH_REDUCE_ONLY)
        ...
        if (std::abs(newDelta - delta) < 1e-15 || lastAction == DECREASED_DELTA)
          stay = false;  // Searching, but Newton's solution is within trust
                         // region so keep the same trust region
        else {
          stay = true;  // Searching and increased delta, so try again to
                        // increase delta
      ...
      if (mode == ONE_STEP_PER_ITERATION ||
          /* mode == SEARCH_EACH_ITERATION && */ lastAction ==
                  INCREASED_DELTA ||
          hitMinimumDelta)
        stay = false;  // If not searching, just return with the new smaller
                       // delta
      ...
      // f actually increased, so keep decreasing delta until f does not
      // decrease. NOTE:  NaN and Inf solutions also will fall into this case,
      // so that we decrease delta if the solution becomes undetermined.
      assert(0.0 > rho);
      if (delta > 1e-5) {
        delta *= 0.5;
        stay = true;
        ...
        if (verbose)
          std::cout << "Warning:  Dog leg stopping because cannot decrease "
                       "error with minimum delta"
                    << std::endl;
        result.dx_d.setZero();  // Set delta to zero - don't allow error to
                                // increase
