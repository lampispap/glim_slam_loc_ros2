...
class IncrementalFixedLagSmootherExtWithFallback
    : public IncrementalFixedLagSmootherExt {
  public:
  IncrementalFixedLagSmootherExtWithFallback(double smootherLag,
  ...
  const GaussianFactorGraph& getLinearFactors() const {
    return smoother->getLinearFactors();
  }
  ...
  Matrix marginalCovariance(Key key) const {
    return smoother->marginalCovariance(key);
  }
