...
struct NumericalGradientDescentStatus {
  std::string to_string() const {
    return (boost::format("%d: error:%.6f -> %.6f lambda:%10g") % iterations %
            error_old % error_new % lambda)
            .str();
  }
...
class NumericalGradientDescentParams : public gtsam::NonlinearOptimizerParams {
  public:
  NumericalGradientDescentParams()
  ...
  std::function<void(const NumericalGradientDescentStatus&,
                     const gtsam::Values&)>
          callback;  // callback for optimization iteration
};

struct NumericalGradientDescentState
    : public gtsam::internal::NonlinearOptimizerState {
  NumericalGradientDescentState(
      ...
      : gtsam::internal::NonlinearOptimizerState(
                initial_values, errors, iterations),
        params(params),
        lambda(lambda) {}

  NumericalGradientDescentState* decrease_lambda(const gtsam::Values& values,
                                                 double error) const {
    return new NumericalGradientDescentState(
            params, lambda / params.lambda_rate, values, error, iterations + 1);
  }

  NumericalGradientDescentState* increase_lambda(const gtsam::Values& values,
                                                 double error) const {
    return new NumericalGradientDescentState(
            params, lambda * params.lambda_rate, values, error, iterations + 1);
  }
...
/**
 * @brief Naive gradient descent with numerical differentiation for objective
 * function test Never use this for practical application!!
 */
class NumericalGradientDescent : public gtsam::NonlinearOptimizer {
  public:
  using State = NumericalGradientDescentState;

  NumericalGradientDescent(const gtsam::NonlinearFactorGraph& graph,
                           const gtsam::Values& initial_value,
                           const NumericalGradientDescentParams& params)
      : gtsam::NonlinearOptimizer(
                graph,
                std::unique_ptr<NumericalGradientDescentState>(
                        new NumericalGradientDescentState(
                                params, params.init_lambda, initial_value))),
        params_(params) {}

  virtual gtsam::GaussianFactorGraph::shared_ptr iterate() override {
    const auto& current_state =
            static_cast<const NumericalGradientDescentState*>(state_.get());
