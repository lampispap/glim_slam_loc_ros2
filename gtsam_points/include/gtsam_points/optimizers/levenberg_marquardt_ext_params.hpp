...
class LevenbergMarquardtExtParams : public gtsam::LevenbergMarquardtParams {
  ...
  LevenbergMarquardtExtParams ensureHasOrdering(
          const gtsam::NonlinearFactorGraph& graph) const;

  void set_verbose() {
    callback = [](const LevenbergMarquardtOptimizationStatus& status,
                  const gtsam::Values&) {
  ...
  std::function<void(const LevenbergMarquardtOptimizationStatus&,
                     const gtsam::Values&)>
