...
class ContinuousTrajectory {
   ...
   */
  ContinuousTrajectory(char symbol,
                       double start_time,
                       double end_time,
  ...
  /**
   * @brief Get an expression of the linear acceleration and angular velocity at
   * t
   * @param t  Time
   * @param t_ Expression of t
   */
  gtsam::Vector6_ imu(double t,
                      const gtsam::Double_& t_,
                      const Eigen::Vector3d& g = Eigen::Vector3d(0.0,
                                                                 0.0,
                                                                 9.80665));
   ...
   * @param t       Time
   */
  gtsam::Vector6 imu(const gtsam::Values& values,
                     double t,
                     const Eigen::Vector3d& g = Eigen::Vector3d(0.0,
                                                                0.0,
                                                                9.80665));
  ...
  /**
   * @brief Optimize spline knots to fit the interpolated trajectory to a set of
   * poses
   * @param stamps      Timestamps of target poses
   * @param poses       Target poses
   * @param smoothness  Smoothness regularization to prevent underconstrained
   * system (If smoothness <= 0.0, regularization will be disabled)
  ...
  gtsam::Values fit_knots(const std::vector<double>& stamps,
                          const std::vector<gtsam::Pose3>& poses,
                          double smoothness = 1e-3,
