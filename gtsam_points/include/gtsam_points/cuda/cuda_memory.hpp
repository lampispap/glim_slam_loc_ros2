...
// Simple wrapper functions to manipulate objects on GPU memory from non-CUDA
// code

void* cuda_malloc(size_t size, CUstream_st* stream = nullptr);

void* cuda_malloc_and_upload(const void* data,
                             size_t size,
                             CUstream_st* stream = nullptr);

void cuda_free(void* ptr, CUstream_st* stream = nullptr);

void cuda_host_to_device(void* dst,
                         const void* src,
                         size_t size,
                         CUstream_st* stream = nullptr);

void cuda_device_to_host(void* dst,
                         const void* src,
                         size_t size,
                         CUstream_st* stream = nullptr);
...
T* cuda_new(const T& data, CUstream_st* stream = nullptr) {
  void* ptr = cuda_malloc_and_upload(reinterpret_cast<const void*>(&data),
                                     sizeof(T), stream);
...
template <typename T>
void cuda_host_to_device(T* dst,
                         const T* src,
                         size_t size,
                         CUstream_st* stream = nullptr) {
  cuda_host_to_device(reinterpret_cast<void*>(dst),
                      reinterpret_cast<const void*>(src), size, stream);
}

template <typename T>
void cuda_device_to_host(T* dst,
                         const T* src,
                         size_t size,
                         CUstream_st* stream = nullptr) {
  cuda_device_to_host(reinterpret_cast<void*>(dst),
                      reinterpret_cast<const void*>(src), size, stream);
}
...
template <typename T>
std::shared_ptr<T> cuda_make_shared(const T& data,
                                    CUstream_st* stream = nullptr) {
  T* ptr = reinterpret_cast<T*>(
          cuda_malloc_and_upload(&data, sizeof(T), stream));
  return std::shared_ptr<T>(ptr, [](T* ptr) { cuda_free(ptr); });
}
